...
description: Experience Platform Web SDK을 사용하여 도메인 간 및 모바일 앱을 웹 브라우저 시나리오에 맞게 Adobe Target을 구성하는 방법에 대해 알아봅니다.
...
Platform Web SDK은 고객이 도메인 간에 개인화된 경험을 보다 정확하게 전달할 수 있도록 하는 방문자 ID 공유 기능을 지원합니다. 이 기능을 사용하면 도메인 간에 일관된 개인화를 제공할 수 있으며 서드파티 쿠키에 의존하지 않고 방문자 활동 보고의 정확도를 향상시킬 수 있습니다.
...
도메인 간 ID 공유를 위해 웹 SDK 버전 2.11.0은 `appendIdentityToUrl` 명령에 대한 지원을 추가합니다. 이 명령을 사용하면 `adobe_mc` 쿼리 문자열 매개 변수가 생성됩니다.
...
>[!TIP]
>
>태그 기능(이전 Launch)을 사용하여 Web SDK을 구현하는 경우, 사용자 지정 코드 없이 도메인 간 ID 공유를 수행할 수 있습니다. 자세한 내용은 [전용 설명서](https://experienceleague.adobe.com/docs/experience-platform/edge/identity/id-sharing.html#tags-extension)를 참조하세요.

>[!NOTE]
>
>Platform Web SDK은 기본 모바일 앱 사용 사례에 대해 모바일-웹 ID 공유도 지원합니다. 자세한 내용은 [Mobile-to-Web 및 도메인 간 ID 공유](https://experienceleague.adobe.com/docs/experience-platform/edge/identity/id-sharing.html)에 대한 전용 설명서를 참조하십시오.

다음으로 Platform Web SDK과의 호환성을 보장하기 위해 [대상 및 프로필 스크립트를 업데이트](update-audiences.md)하는 방법을 알아봅니다.
