...
1. 구현을 업데이트하여 Target SDK을 XYZ로 교체합니다.
1. 특정 사용 사례에 맞게 Platform Mobile SDK 구현을 개선합니다. 이는 추가 파라미터, XYZ를 전달하는 것을 포함할 수 있다.
1. 프로필 스크립트, 활동 및 대상 정의 와 같은 Target 인터페이스의 객체를 업데이트합니다???
1. 업데이트된 앱을 게시하기 전에 최종 구현의 유효성 검사
...
|---|---|---|
| 소유권 | Target 확장은 다른 애플리케이션 SDK와 독립적입니다(모바일에는 해당되지 않는다고 생각함). 서로 다른 이러한 라이브러리의 사용자 지정 및 소유권은 조직의 다른 팀에 맞출 수 있습니다. | Platform Web SDK 라이브러리 및 전달된 데이터는 모든 Adobe 애플리케이션에 대해 통합됩니다. Platform Web SDK 구현의 소유권은 모든 다운스트림 애플리케이션의 관련자를 나타내야 합니다. |
| 유지 관리 | Target 및 Analytics와 같은 각 Adobe 애플리케이션에 대해 별도의 팀이 구현 개선 작업을 수행할 수 있습니다. | 이상적으로 단일 팀이 Platform Web SDK 구현에 영향을 주는 향상된 기능을 담당해야 합니다. |
...
Platform Web SDK은 Target at.js 라이브러리의 진화가 아닙니다. 웹 채널에 대한 모든 Adobe 애플리케이션을 구현하기 위한 새롭고 통합된 방식입니다. 알아야 할 몇 가지 기술적인 차이점이 있습니다.

| | Target at.js 2.x | Platform Web SDK |
|---|---|---|
| 라이브러리 기능 | at.js에서 제공하는 Target 기능입니다. Visitor.js 및 AppMeasurement.js에서 제공하는 다른 애플리케이션과의 통합 | 단일 플랫폼 웹 SDK 라이브러리에서 제공하는 모든 Adobe 애플리케이션에 대한 기능: alloy.js |
