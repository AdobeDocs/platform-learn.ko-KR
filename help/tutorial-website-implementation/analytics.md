...
당신은 이미 의 수업을 끝냈어야 했다 [태그 구성](create-a-property.md) 및 [ID 서비스 추가](id-service.md).
...
1. 아래 [!UICONTROL 라이브러리 관리 > 보고서 세트]를 입력하면 각 태그 환경에 사용할 보고서 세트 id를 입력합니다. 사용자가 Adobe Analytics에 액세스할 수 있는 경우 상자에 입력을 시작하면 모든 보고서 세트의 미리 채워진 목록이 표시됩니다. (이 자습서에서는 모든 환경에 대해 하나의 보고서 세트를 사용할 수 있지만 실제 환경에서는 아래 이미지와 같이 별도의 보고서 세트를 사용합니다.)

   ![보고서 세트 ID 입력](images/analytics-config-reportSuite.png)

   >[!TIP]
   >
   > [!UICONTROL  라이브러리를 더 쉽게 최신 상태로 유지할 수 있도록 ]Library Management[!UICONTROL  설정으로 ]Manage the library for me 옵션`AppMeasurement.js`을 사용하는 것이 좋습니다.

1. 아래 [!UICONTROL 일반 > 추적 서버]를 입력하여 추적 서버를 입력합니다(예: ). `tmd.sc.omtrdc.net`. 사이트가 `https://`를 지원하는 경우 SSL 추적 서버를 입력합니다.

   ![추적 서버 입력](images/analytics-config-trackingServer.png)

1. 에서 [!UICONTROL 전역 변수 섹션], 아래에 [!UICONTROL 추가 설정], 설정 [!UICONTROL 페이지 이름] 변수를 `Page Name` 데이터 요소를 생성하지 않습니다. ![data element 아이콘](images/icon-dataElement.png) 아이콘을 클릭하여 모달을 열고 페이지 `Page Name` 데이터 요소를 선택합니다.
...
>[!NOTE]
>
>글로벌 변수는 확장 구성 또는 규칙 작업에서 설정할 수 있습니다. 확장 구성에서 변수를 설정할 때 데이터 계층을 정의해야 합니다 *이전* 태그 포함 코드.

## 페이지 보기 비콘 보내기

이제 Analytics 비콘이 실행되는 규칙을 만들겠습니다. 이 규칙은 확장 구성에 설정된 [!UICONTROL 페이지 이름] 변수를 전송합니다.

이미 의 &quot;All Pages - Library Loaded&quot; 규칙을 만들었습니다 [데이터 요소, 규칙 및 라이브러리 추가](add-data-elements-rules.md) 태그 라이브러리가 로드될 때 모든 페이지에서 트리거되는 이 자습서의 단원입니다. 사용자 *가능하* 이 규칙은 Analytics에도 사용합니다. 하지만 이 설정을 사용하려면 Analytics 비콘에 사용된 모든 데이터 계층 속성을 태그 포함 코드 앞에 정의해야 합니다. 데이터 수집을 보다 유연하게 하려면, DOM 지원에서 트리거된 새로운 &quot;모든 페이지&quot; 규칙을 만들어 Analytics 비콘을 실행합니다.

**페이지 보기 비콘을 보내려면**

1. 로 이동합니다. **[!UICONTROL 규칙]** 왼쪽 탐색 영역에서 섹션을 클릭한 다음 을 클릭합니다 **[!UICONTROL 규칙 추가]**

   ![규칙 추가](images/analytics-addRule.png)

1. 규칙 이름을 지정합니다 `All Pages - DOM Ready`
1. **[!UICONTROL Events > Add]**&#x200B;를 클릭하여 `Event Configuration` 화면을 엽니다.

   ![규칙에 이름 지정 및 이벤트 추가](images/analytics-domReady-nameAddAnalyticsEvent.png)

1. **[!UICONTROL 이벤트 유형 > DOM Ready]**&#x200B;를 선택합니다(이 규칙의 순서는 &quot;50&quot;).
...
1. **[!UICONTROL Extension > Adobe Analytics]**&#x200B;를 선택합니다.

1. **[!UICONTROL Action Type > Send Beacon]**&#x200B;을 선택합니다.
...
1. 디버거 아이콘 ![Experience Cloud Debugger 열기](images/analytics-debuggerIcon.png)을 클릭하여 **[!UICONTROL Adobe Experience Cloud Debugger]**&#x200B;를 엽니다.
...
Analytics 확장을 구성할 때 확장 구성에서 `pageName` 변수를 입력했습니다. 이 위치는 태그 포함 코드가 로드되기 전에 페이지에서 이 값을 사용할 수 있는 경우 eVar 및 prop과 같은 다른 글로벌 변수를 입력하기에 적합합니다.
...
1. 사용 `digitalData.page.category.type` 로서의 **[!UICONTROL JavaScript 변수 이름]**
1. 을(를) 확인합니다. **[!UICONTROL 텍스트 정리]** 및 **[!UICONTROL 소문자 강제 적용]** 옵션
1. **[!UICONTROL Save to Libray]**&#x200B;를 클릭합니다.
...
1. **[!UICONTROL Data Element Type > JavaScript Variable]**&#x200B;를 선택합니다.
1. 사용 `digitalData.product.0.productInfo.sku` 로서의 **[!UICONTROL JavaScript 변수 이름]**
1. 을(를) 확인합니다. **[!UICONTROL 텍스트 정리]** 및 **[!UICONTROL 소문자 강제 적용]** 옵션
1. **[!UICONTROL Save to Libray]**&#x200B;를 클릭합니다.
...
1. **[!UICONTROL Event Type > DOM Ready]**&#x200B;를 선택합니다.
1. 규칙이 ytics > Send Beacon 작업이 포함된 규칙보다 **[!UICONTROL 먼저]** 실행되도록 *주문*&#x200B;을 40으로 설정합니다.
1. **[!UICONTROL Keep Changes]**를 클릭합니다.

   ![이벤트 구성](images/analytics-configDOMReadyEvent.png)

1. **[!UICONTROL Conditions]**&#x200B;에서 ![Click the Plus 아이콘](images/icon-plus.png)을 클릭하여 `Condition Configuration` 화면을 엽니다.
   ![Plus 아이콘을 클릭하여 새 조건 추가](images/analytics-PDPRuleAddCondition.png)

   1. **[!UICONTROL Condition Type > Value Comparison]**&#x200B;를 선택합니다.
   1. 데이터 요소 선택기를 사용하여 첫 번째 필드에서 `Page Type`을 선택합니다.
   1. 비교 연산자 드롭다운에서 **[!UICONTROL Contains]**&#x200B;를 선택합니다.
...
1. **[!UICONTROL Extension > Adobe Analytics Product String]**&#x200B;을 선택합니다.
1. **[!UICONTROL Action Type > Set s.products]**&#x200B;를 선택합니다.

1. **[!UICONTROL Analytics E-commerce Event]** 섹션에서 **[!UICONTROL prodView]**&#x200B;를 선택합니다.

1. **[!UICONTROL Data layer variables for product data]** 섹션에서 데이터 요소 선택기를 사용하여 `Product Id` 데이터 요소를 선택합니다.
...
1. **[!UICONTROL Extension > Adobe Analytics]**&#x200B;를 선택합니다.
1. **[!UICONTROL Action Type > Set Variables]**&#x200B;를 선택합니다.
1. **[!UICONTROL eVar1 > Set as]**&#x200B;를 선택하고 `product detail page`를 입력합니다.
1. **[!UICONTROL event1]**&#x200B;을 설정하고, 선택적 값을 비워둡니다.
1. Events에서 **[!UICONTROL Add Another]** 단추를 클릭합니다.
...
하지만 수행하는 작업이 페이지 보기보다 &quot;더 작으므로&quot;(또는 페이지 보기와 다를 수 있으므로), 사이트에서 페이지 보기를 늘리고 싶지 않은 경우가 있을 수 있습니다. 이런 경우에는 `s.tl()` 함수를 사용합니다. 이 함수를 일반적으로 &quot;추적 링크&quot; 요청이라고도 합니다. 이것을 추적 링크 요청이라고 하지만, 링크 클릭 시 트리거되지 않아도 됩니다. 에 의해 트리거될 수 있습니다. *임의* 고유한 사용자 지정 JavaScript를 포함하여 태그 규칙 빌더에서 사용할 수 있는 이벤트 .

이 자습서에서는 가장 멋진 JavaScript 이벤트 중 하나인 `Enters Viewport` 이벤트를 사용하여 `s.tl()` 호출을 트리거합니다.

### 사용 사례

이 사용 사례에서는 사람들이 Luma 홈 페이지에서 아래로 스크롤하여 을 볼 수 있는지 알려고 합니다. *주요 제품* 섹션을 참조하십시오. 사람들이 해당 섹션을 보고 있는지 여부에 대한 사내 의견 불일치가 있으므로 Analytics를 사용하여 진실을 파악하고자 합니다.
...
1. **[!UICONTROL Event Type > Enters Viewport]**&#x200B;를 선택합니다. 이렇게 하면 CSS 선택기를 입력해야 하는 필드가 표시됩니다. 이 선택기는 페이지에서 규칙이 브라우저에서 뷰에 들어올 때 규칙을 트리거해야 하는 항목을 식별합니다.
1. Luma의 홈 페이지로 돌아가 중요 제품 섹션으로 스크롤합니다.
1. 주요 제품 제목과 이 섹션의 항목 사이에 있는 공백을 마우스 오른쪽 버튼으로 클릭하고 를 선택합니다 `Inspect` 마우스 오른쪽 단추 클릭 메뉴에서 을 클릭합니다. 이렇게 하면 원하는 내용에 가까워집니다.
...
1. 이 요소를 마우스 오른쪽 단추로 클릭하고 **[!UICONTROL Copy > Copy Selector]**&#x200B;를 선택합니다.

   ![뷰포트에 들어갑니다 이벤트 구성](images/analytics-copyElementSelector.png)

1. 태그로 돌아가서 클립보드의 이 값을 라는 레이블이 지정된 필드에 붙여 넣습니다 `Elements matching the CSS selector`.
...
1. 비교 연산자 드롭다운에서 **[!UICONTROL 같음]**&#x200B;을 선택합니다.
...
1. **[!UICONTROL Extension > Adobe Analytics]**&#x200B;를 선택합니다.
1. **[!UICONTROL Action Type > Send Beacon]**&#x200B;을 선택합니다.
1. 을(를) 선택합니다 **[!UICONTROL `s.tl()`]** 추적 옵션
1. **[!UICONTROL Link Name]** 필드에 `Scrolled down to Featured Products`를 입력합니다. 이 값은 Analytics의 사용자 지정 링크 보고서에 입력됩니다.
1. **[!UICONTROL Keep Changes]**&#x200B;를 클릭합니다.

   ![중요 제품 비콘 구성](images/analytics-configEntersViewportBeacon.png)
...
1. Debugger를 연 상태에서 중요 제품 섹션이 표시될 때까지 사이트에서 아래로 스크롤합니다.
...
1. **[!UICONTROL Extensions > Installed]**&#x200B;로 이동합니다.

1. Adobe Analytics 확장에서 **[!UICONTROL Configure]**&#x200B;을 클릭합니다.

   ![Analytics 구성](images/analytics-configureExtension.png)

1. **[!UICONTROL 라이브러리 관리]**&#x200B;에서 `Make tracker globally accessible`이라는 레이블이 있는 상자를 선택합니다. 도움말 풍선에서 볼 수 있듯이 이렇게 하면 window.s에서 전체적으로 추적기를 사용할 수 있으며, 이것은 고객 JavaScript에서 추적기를 참조할 때 매우 중요합니다.
   ![추적기에 전체적으로 액세스 가능](images/analytics-makeTrackerGlobal.png)
