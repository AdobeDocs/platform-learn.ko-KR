...
Adobe Target 확장은 Target의 at.js JavaScript 라이브러리를 사용하여 클라이언트측 구현을 지원합니다. Adobe Target의 웹 SDK 구현은 [Experience Platform 웹 SDK 자습서](https://experienceleague.adobe.com/ko/docs/platform-learn/implement-web-sdk/applications-setup/setup-target)를 참조하십시오.
...
1. `pageName`이름&#x200B;**[!UICONTROL (으)로]** 입력
...
1. **[!UICONTROL 이(가) 전역 범위에 선언되도록]**&#x200B;전역적으로 실행`targetPageParams()` 상자를 선택합니다.
1. **[!UICONTROL 변경 내용 유지]** 클릭
...
>[!WARNING]
>
>`at_property`페이지 로드 요청에 매개 변수 추가&#x200B;**[!UICONTROL 작업을 통해]** 매개 변수를 추가하려고 하면 매개 변수가 네트워크 요청에 채워지지만 페이지를 로드할 때 Target의 VEC(시각적 경험 작성기)에서 이 매개 변수를 자동으로 검색할 수 없습니다. 항상 사용자 지정 코드 작업에서 `at_property` 함수를 사용하여 `targetPageParams()`을(를) 채우십시오.
...
1. **[!UICONTROL 이벤트 > 추가]** 클릭
   ...
   1. **[!UICONTROL 고급 옵션]**&#x200B;에서 `Order`을(를) `60`(으)로 변경하여 `Load Target` 작업(`All Pages - Library Loaded`이 `Order`(으)로 설정된 `50` 규칙에 있음) 후에 실행되도록 합니다.
   1. **[!UICONTROL 변경 내용 유지]** 클릭
