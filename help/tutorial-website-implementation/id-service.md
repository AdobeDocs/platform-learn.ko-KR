---
title: 태그와 함께 Adobe Experience Platform Identity 서비스 추가
...
[Adobe Experience Platform ID 서비스](https://experienceleague.adobe.com/docs/id-service/using/home.html)는 솔루션 간 대상 공유와 같은 Experience Cloud 기능을 작동시키기 위해 모든 Adobe 솔루션에 대해 공통인 방문자 ID를 설정합니다. 또한 고유한 고객 ID를 서비스로 보내어 장치 간 타깃팅 및 CRM(고객 관계 관리) 시스템과의 통합을 가능하게 할 수 있습니다.
...
이미 의 단원을 완료했어야 합니다. [태그 구성](create-a-property.md) 섹션을 참조하십시오.
...
1. 왼쪽 탐색에서 **[!UICONTROL 확장]**

1. **[!UICONTROL Catalog]**&#x200B;를 클릭하여 Extensions Catalog 페이지로 이동합니다.

1. 카탈로그에서 사용할 수 있는 다양한 확장을 확인합니다.

1. 맨 위의 필터에서 &quot;id&quot;를 입력하여 카탈로그를 필터링합니다.

1. Adobe Experience Platform ID 서비스용 카드에서 **[!UICONTROL Install]**&#x200B;을 클릭합니다.

   ![ID 서비스 확장 설치](images/idservice-install.png)

1. Experience Cloud 조직 ID가 자동으로 검색됩니다.

1. 모든 기본 설정을 그대로 두고 **[!UICONTROL Save to Library and Build]**&#x200B;를 클릭합니다.
...
1. 디버거의 요약 탭에서 태그 섹션은 Adobe Experience Platform Identity 서비스 확장이 구현되었음을 나타냅니다.
...
다음으로, [고객 ID](https://experienceleague.adobe.com/docs/id-service/using/reference/authenticated-state.html?lang=ko-KR)를 ID 서비스로 보냅니다. 이렇게 하면 Experience Cloud와 [CRM을 통합](https://experienceleague.adobe.com/docs/core-services/interface/customer-attributes/attributes.html?lang=ko-KR)하고 장치 간에 방문자를 추적할 수 있습니다.
...
1. **[!UICONTROL Add Data Element]** 단추를 클릭합니다.

   ![&quot;Add Data Element&quot; 클릭](images/idservice-addDataElement1.png)

1. 데이터 요소에 이름을 지정합니다 `Authentication State`
1. **[!UICONTROL Data Element Type]**&#x200B;에 대해 **[!UICONTROL Custom Code]**&#x200B;를 선택합니다.
1. **[!UICONTROL Open Editor]** 단추를 클릭합니다.
...
1. 클릭 **[!UICONTROL 라이브러리에 저장]** 데이터 요소를 저장하고 데이터 요소 페이지로 돌아가려면 를 클릭합니다. 모든 변경 작업을 수행하고 유효성을 검사할 준비가 될 때까지 &quot;빌드&quot;를 수행할 필요가 없습니다.
...
1. **[!UICONTROL Data Element Type]**&#x200B;에 대해 **[!UICONTROL JavaScript Variable]**&#x200B;를 선택합니다.
1. **[!UICONTROL JavaScript variable name]**&#x200B;으로서 Luma 사이트의 데이터 레이어에 있는 변수에 다음 포인터를 사용합니다. `digitalData.user.0.profile.0.attributes.username`
1. 다른 모든 설정은 기본값으로 둡니다.
1. 클릭 **[!UICONTROL 라이브러리에 저장]** 데이터 요소를 저장하려면
...
1. **[!UICONTROL Add Rule]**&#x200B;을 클릭하여 규칙 빌더를 엽니다.
...
1. **[!UICONTROL Events]** 아래에서 **[!UICONTROL Add]**&#x200B;를 클릭합니다.

   ![이벤트 추가](images/idservice-customerId-addEvent.png)

   1. **[!UICONTROL Event Type]**&#x200B;으로서 **[!UICONTROL Library Loaded (Page Top)]**&#x200B;을 선택합니다.
   1. 를 확장합니다. **[!UICONTROL 고급 옵션]** 및  **[!UICONTROL 주문]** enter `10`. 이 순서는 동일한 이벤트에 의해 트리거되는 규칙의 순서를 제어합니다. 순서가 낮은 규칙이 순서가 높은 규칙보다 먼저 실행됩니다. 이 경우 Target 요청을 실행하기 전에 고객 ID를 설정해야 합니다. 이 작업은 순서가 `50`인 규칙을 사용하여 다음 단원에서 수행합니다.
   1. **[!UICONTROL Keep Changes]** 단추를 클릭하여 규칙 빌더로 돌아갑니다.

   ![이벤트 저장](images/idservice-customerId-saveEvent.png)

1. **[!UICONTROL Conditions]** 아래에서 **[!UICONTROL Add]**&#x200B;를 클릭합니다.

   ![규칙에 조건 추가](images/idservice-customerId-addCondition.png)

   1. **[!UICONTROL Condition Type]**&#x200B;에 대해 **[!UICONTROL Value Comparison]**&#x200B;을 선택합니다.
   ...
   1. 데이터 요소 모달에서 인증 **[!UICONTROL Authentication State]**&#x200B;를 클릭한 다음, **[!UICONTROL Select]**&#x200B;를 클릭합니다.
...
1. **[!UICONTROL Actions]** 아래에서 **[!UICONTROL Add]**&#x200B;를 클릭합니다.

   ![새 작업 추가](images/idservice-customerId-addAction.png)

   1. 대상 **[!UICONTROL 확장]** 선택 **[!UICONTROL Experience Cloud ID 서비스]**
   1. **[!UICONTROL Action Type]**&#x200B;에 대해 **[!UICONTROL Set Customer IDs]**&#x200B;를 선택합니다.
   1. **[!UICONTROL Integration Code]**&#x200B;에 대해 `crm_id`를 입력합니다.
   1. **[!UICONTROL Value]**&#x200B;에 대해 데이터 요소 선택기 모달을 열고 `Email (Hashed)`를 선택합니다.
   1. **[!UICONTROL Auth State]**&#x200B;에 대해 **[!UICONTROL Authenticated]**&#x200B;를 선택합니다.
   1. **[!UICONTROL Keep Changes]** 단추를 클릭하여 작업을 저장하고 규칙 빌더로 돌아갑니다.

      ![작업 구성 및 변경 내용 저장](images/idservice-customerId-action.png)

1. **[!UICONTROL Save to Library and Build]** 단추를 클릭하여 규칙을 저장합니다.
...
태그에는 풍부한 콘솔 로깅 기능도 있습니다. 켜려면 **[!UICONTROL 도구]** 디버거에서 탭을 열고 **[!UICONTROL 태그 콘솔 로깅]** 토글.
