...
이 설명서에 따라 Journey Optimizer 사용자는 offer decisioning 기능을 효과적으로 적용할 수 있게 되어 고객 상호 작용의 개인화와 관련성을 향상시킵니다.
...
* offer decisioning을 위해 Web SDK를 구성하여 AJO와 원활하게 통합하기 위한 단계별 프로세스에 대해 알아봅니다.
...
* 조직에서 Adobe Journey Optimizer Ultimate(AJO 및 Offer decisioning) 또는 Adobe Experience Platform과 Offer decisioning 애플리케이션 서비스 추가 기능에 액세스할 수 있는지 확인합니다.
...
1. 다음으로 이동 [데이터 수집](https://experience.adobe.com/#/data-collection) 인터페이스.
...
>[!NOTE]
>
> 이 튜토리얼은 구현자를 대상으로 하므로 이 단원에는 AJO의 실질적인 UI 작업이 포함되어 있습니다. 이러한 UI 작업은 일반적으로 마케터가 처리하지만, 구현자가 장기적으로 의사 결정 관리 캠페인 생성에 대한 책임이 없어도 프로세스에 대한 통찰력을 얻는 것이 유익할 수 있습니다.
...
1. 결정 범위 내의 구성에 필요할 수 있으므로 배치 ID를 참고하십시오.
...
1. 첫 번째 규칙 이름을 &#39; 로 지정합니다.*Gold 충성도 상태 규칙*&#39;. XDM 필드를 사용하여 규칙을 정의할 수 있습니다. 더 Adobe Experience Platform **세그먼트 빌더** 는 규칙 조건을 작성하는 데 사용할 수 있는 직관적인 UI입니다.
