...
XDM은 디지털 경험의 성능을 개선하기 위해 설계된 공개적으로 문서화된 사양입니다. 모든 애플리케이션이 플랫폼 서비스와 통신할 수 있도록 하는 일반적인 구조와 정의를 제공합니다. XDM 표준을 준수함으로써 모든 고객 경험 데이터를 일반적인 표현에 통합하여 보다 빠르고 통합적인 방식으로 통찰력을 제공할 수 있습니다. 고객 작업에서 중요한 통찰력을 얻고, 세그먼트를 통해 고객 대상을 정의하고, 개인화 목적으로 고객 속성을 표현할 수 있습니다.
...
1. 앱 전환기를 연 다음 을 선택합니다. **[!UICONTROL 데이터 수집]**
...
학습 목적으로 사전 빌드된 필드 그룹 및 사용자 정의 필드 그룹을 사용합니다.
...
1. 선택 **[!UICONTROL 스키마 만들기]** options 드롭다운 메뉴를 가져온 다음 를 선택합니다. **[!UICONTROL XDM ExperienceEvent]**.

   ![드롭다운에서 ExperienceEvent 선택](assets/mobile-schema-create.png)

1. `Consumer Experience Event`을 검색합니다. 

1. 선택하기 전에 필드를 미리 보거나 자세한 내용을 보려면 설명을 읽을 수 있습니다.
...
1. 을 선택하여 스키마 이름 지정 **[!UICONTROL 제목 없는 스키마]** 왼쪽 상단에서 **[!UICONTROL 표시 이름]** 및 **[!UICONTROL 설명]**, 예 `Luma Tutorial Mobile` 및 `"Luma App" schema for Adobe Tutorial`
...
>[!NOTE]
>
>그룹의 모든 필드를 사용할 필요는 없습니다. 유용한 경우 스키마를 빈 데이터 레이어로 생각할 수 있습니다. 앱에서 적절한 시간에 관련 값을 채웁니다.
>
>다음 `Consumer Experience Event` 이(가) (이)라는 데이터 형식을 갖습니다. `Web information`페이지 보기 및 링크 클릭과 같은 이벤트에 대해 설명합니다. 작성 시에는 이 기능에 대한 모바일 앱 패리티가 없으므로 직접 만듭니다.
...
1. 다음 항목 선택 **[!UICONTROL 데이터 유형]** 탭을 선택한 다음 를 선택합니다 **[!UICONTROL 데이터 유형 만들기]**.
...
1. 다음을 제공합니다. **[!UICONTROL 표시 이름]** 및 **[!UICONTROL 설명]**, 예 `App Information` 및 `Custom data type describing "Screen Views" & "App Actions"`
   ...
   ![이름 및 설명 입력](assets/mobile-schema-datatype-name.png)
...
1. 필드를 추가하려면 (+) 버튼을 선택합니다.
   ...
   이 필드는 앱 상호 작용을 위한 컨테이너 개체입니다. 낙타 케이스로 주세요 **[!UICONTROL 필드 이름]** `appInteraction`, **[!UICONTROL 표시 이름]** `App Interaction`, 및 **[!UICONTROL 유형]** `Object`.

1. **[!UICONTROL 적용]**&#x200B;을 선택합니다.

   ![새 앱 작업 이벤트 추가](assets/mobile-schema-datatype-app-action.png)

1. 작업이 발생하는 빈도를 측정하려면 옆에 있는 (+) 버튼을 선택하여 필드를 추가합니다. `appInteraction` 개체를 만들었습니다.
...
1. 다음 옆에 있는 (+) 버튼을 선택하여 상호 작용 유형을 설명하는 필드를 추가합니다. `appInteraction` 개체.
...
1. 동일한 패턴을 따라 `appStateDetails` 라는 측정값 필드가 포함된 개체 `screenView` 및 2개의 문자열이 호출됨 `screenName` 및 `screenType`.
...
1. 선택 **[!UICONTROL 추가]** 다음에 **[!UICONTROL 필드 그룹]**.

   ![새 필드 그룹 추가 중](assets/mobile-schema-fieldgroup-add.png)

1. 이번에는 다음을 선택하여 사용자 정의 필드 그룹을 만듭니다. **[!UICONTROL 새 필드 그룹 만들기]** 상단 근처에 있는 라디오 버튼을 클릭한 다음 이름과 설명을 입력합니다(예: ). `App Interactions` 및 `Fields for app interactions`.

   ![이름 및 설명 입력](assets/mobile-schema-fieldgroup-name.png)

1. 기본 작성 화면에서 스키마 루트에 필드를 추가합니다.

1. 스키마 이름 옆에 있는 (+)를 선택합니다.

1. 오른쪽 레일에서 **[!UICONTROL 필드 이름]** / `appInformation`, 표시 이름 `App Information`.

1. 선택 `App Information` 다음에서 **[!UICONTROL 유형]** 드롭다운은 이전 연습에서 만든 데이터 유형입니다.
