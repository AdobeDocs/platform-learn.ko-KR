---
title: Platform Mobile SDK으로 WebViews 처리
...
**[!DNL Luma]** > **[!DNL Luma]** > **[!DNL Views]** > **[!DNL Info]** > **[!DNL TermsOfServiceSheet]**(으)로 이동하여 `func loadUrl()` 클래스에서 `final class SwiftUIWebViewModel: ObservableObject` 함수를 찾습니다. 웹 보기를 처리하기 위해 다음 호출을 추가합니다.
...
**[!UICONTROL Android]** ![VDown](/help/assets/icons/ChevronDown.svg) > **[!DNL app]** > **[!DNL kotlin+java]** > **[!DNL com.adobe.luma.tutorial.android]** > **[!DNL views]** > **[!DNL WebViewModel]**(으)로 이동하여 `fun loadUrl()`에서 `class WebViewModel: ViewModel()` 함수를 찾습니다. 웹 보기를 처리하기 위해 다음 호출을 추가합니다.
...
1. 이벤트를 선택하고 **[!UICONTROL ACPExtensionEventData]** 개체에서 **[!UICONTROL urlvariable]** 필드를 검토하여 다음 매개 변수가 URL에 있는지 확인합니다. `adobe_mc`, `mcmid` 및 `mcorgid`.
   ...
   ![웹 보기 유효성 검사](assets/webview-validation.png){zoomable="yes"}
...
>[!NOTE]
>
>이러한 URL 매개 변수를 통한 방문자 결합은 Platform Web SDK(버전 2.11.0 이상) 및 `VisitorAPI.js`을(를) 사용할 때 지원됩니다.
