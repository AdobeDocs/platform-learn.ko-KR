...
![아키텍쳐](assets/architecture-aa.png)
...
Edge Network에서 Adobe Analytics으로 XDM 데이터를 전송하려면 [데이터 스트림 만들기](create-datastream.md)의 일부로 설정한 데이터 스트림에 Adobe Analytics 서비스를 구성합니다.
...
자세히 살펴보면 모든 이벤트에는 두 개의 필드 `value`(필수)과 `id`(선택 사항)이 있습니다. `value` 필드는 이벤트 수를 늘리는 데 사용됩니다. `id` 필드는 serialization에 사용됩니다.
...
* **[!UICONTROL 이벤트]**&#x200B;은(는) `commerce.productListAdds`을(를) 기준으로 `scAdd`(으)로 채워집니다.
...
>
>`_techmarketingdemos`이(가) 조직의 고유 값으로 대체되었습니다.
...
![분석 처리 규칙](assets/analytics-processing-rules.png)
...
다음: **[Experience Platform에게 데이터 보내기](platform.md)**
