---
title: Platform Mobile SDK를 사용하여 프로필 데이터 수집
...
1. Xcode 프로젝트 탐색기에서 **[!DNL Luma]** > **[!DNL Luma]** > **[!DNL Utils]** > **[!DNL MobileSDK]**(으)로 이동하여 `func updateUserAttribute(attributeName: String, attributeValue: String)` 함수를 찾습니다. 다음 코드를 추가합니다.
   ...
   1. `profileMap` 사전을 [`UserProfile.updateUserAttributes`](https://developer.adobe.com/client-sdks/documentation/profile/api-reference/#updateuserattributes) API 호출의 `attributeDict` 매개 변수에 대한 값으로 사용합니다.
...
1. Xcode 프로젝트 탐색기에서 **[!DNL Luma]** > **[!DNL Luma]** > **[!DNL Views]** > **[!DNL General]** > **[!DNL HomeView]**(으)로 이동하여 `.onAppear` 수정자를 찾습니다. 다음 코드를 추가합니다.
   ...
   1. `attributeNames` 배열에서 단일 요소로 `isPaidUser` 특성 이름이 있는 [`UserProfile.getUserAttributes`](https://developer.adobe.com/client-sdks/documentation/profile/api-reference/#getuserattributes) API를 호출합니다.
   1. 그런 다음 `isPaidUser` 특성의 값을 확인하고 `yes`이(가) 오른쪽 상단의 도구 모음에 있는 <img src="assets/paiduser.png" width="20" /> 아이콘

추가 설명서는 [여기](https://developer.adobe.com/client-sdks/documentation/profile/api-reference/#getuserattributes)에서 찾을 수 있습니다.
...
1. Assurance UI에 업데이트된 `profileMap` 값이 있는 **[!UICONTROL UserProfileUpdate]** 및 **[!UICONTROL getUserAttributes]** 이벤트가 표시됩니다.
   ![프로필 유효성 검사](assets/profile-validate.png)

>[!SUCCESS]
>
>이제 Adobe Experience Platform을 사용하여 Edge Network 및 (설정 시) 프로필의 속성을 업데이트하도록 앱을 설정했습니다.
>
