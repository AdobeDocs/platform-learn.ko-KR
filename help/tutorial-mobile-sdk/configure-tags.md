...
>[!NOTE]
>
> Platform launch(Client Side)가 이제 [태그](https://experienceleague.adobe.com/docs/experience-platform/tags/home.html?lang=ko-KR)입니다.
...
1. 라이브러리 만들기:
   ...
   1. **[!UICONTROL 라이브러리 추가]**&#x200B;를 선택합니다.
      ...
      ![라이브러리 추가 선택](assets/tags-create-library.png)
...
1. 확장 확인:
   ...
   1. **[!UICONTROL 설치됨]** 탭을 선택합니다.
      ...
      ![설치된 태그](assets/tags-installed.png)
...
1. ![검색](https://spectrum.adobe.com/static/icons/workflow_18/Smock_Search_18_N.svg) **[!UICONTROL 검색]** 필드를 사용하여 **Adobe Experience Platform Edge Network** 확장을 찾아 설치합니다.
   ...
   1. **[!UICONTROL 데이터스트림]**&#x200B;에서 각 환경(예: **[!DNL Luma Mobile App]**)에 대해 [이전 단계](create-datastream.md)에서 만든 **[!UICONTROL 데이터스트림]**&#x200B;을(를) 선택합니다.

   1. 아직 채우지 않은 경우 **[!UICONTROL Edge Network 구성]**&#x200B;에서 **[!UICONTROL 도메인]**&#x200B;을(를) 지정하십시오. Edge Network 도메인은 조직의 이름이며 `data.adobedc.net`(예: `techmarketingdemos.data.adobedc.net`)이 옵니다.

   1. **[!UICONTROL 라이브러리에 저장]** 메뉴에서 **[!UICONTROL 라이브러리 및 빌드에 저장]**&#x200B;을 선택합니다.

      ![에지 네트워크 설정](assets/tags-extension-edge.png)

라이브러리는 새로운 확장 및 구성을 위해 빌드되었습니다. **[!UICONTROL 초기 빌드]** 단추에 <span style="color:green">●</span>(으)로 빌드가 성공적으로 표시되었습니다.
...
1. CocoaPod를 사용하여 프로젝트를 설정하는 지침을 ![복사](https://spectrum.adobe.com/static/icons/workflow_18/Smock_Copy_18_N.svg)할 수 있습니다. CocoaPod는 SDK 버전 및 다운로드를 관리하는 데 사용됩니다. 자세한 내용은 [CocoaPod 설명서](https://cocoapods.org/)를 참조하십시오. Android™을 개발 플랫폼으로 사용하는 경우 Gradle은 SDK 버전, 다운로드 및 종속성을 관리하는 도구입니다. 자세한 내용은 [Gradle 설명서](https://gradle.org/)를 참조하십시오.

   설치 지침은 구현을 위한 좋은 시작점을 제공합니다. 추가 정보는 [여기](https://developer.adobe.com/client-sdks/documentation/getting-started/get-the-sdk/)에서 찾을 수 있습니다.

   >[!INFO]
   >
   >이 자습서의 나머지 부분에서 CocoaPods 지침을 대신 기본 SPM(Swift Package Manager) 기반 설정을 사용하여 **하지**&#x200B;합니다.
   >

1. **[!UICONTROL 초기화 코드 추가]** 아래에서 **[!UICONTROL Swift]** 탭을 선택하십시오. 이 코드 블록은 launch에서 필요한 SDK를 가져오고 확장을 등록하는 방법을 보여 줍니다. 자세한 내용은 [SDK 설치](install-sdks.md)를 참조하세요.

1. **[!UICONTROL 환경 파일 ID]**&#x200B;를 ![복사](https://spectrum.adobe.com/static/icons/workflow_18/Smock_Copy_18_N.svg)하고 나중에 필요한 위치에 저장합니다. 이 고유 ID는 개발 환경을 가리킵니다. 각 환경(프로덕션, 스테이징, 개발)에는 고유한 ID 값이 있습니다.
