...
description: Workfront Fusion 내에서 Adobe API 사용
kt: 5342
doc-type: tutorial
...
**Photoshop 텍스트 변경** 노드에서 **검색** 아이콘을 클릭하여 응답을 확인합니다. 상태 파일에 대한 링크와 함께 이와 같은 응답이 있어야 합니다.

![WF Fusion](./images/wffusion76.png)

Photoshop API 상호 작용을 계속하기 전에 **Firefly T2I** 노드에 대한 경로를 비활성화하여 해당 API 끝점에 불필요한 API 호출을 보내지 않도록 하겠습니다. **렌치** 아이콘을 클릭한 다음 **경로 비활성화**&#x200B;를 선택합니다.
...
**여러 변수 설정** 노드를 클릭하고 **항목 추가**&#x200B;를 선택합니다. 이전 요청의 응답에서 변수 값을 선택합니다.
...
을(를) 클릭하여 HTTP 요청을 엽니다. 이전 단계에서 만든 변수를 참조하도록 URL을 변경하고 **메서드**&#x200B;을(를) **GET**(으)로 설정합니다.
...
그러면 상태가 **실행 중**(으)로 설정된 필드 **status**&#x200B;이(가) 포함된 응답을 받게 됩니다. Photoshop이 프로세스를 완료하는 데 몇 초 정도 소요됩니다.

![WF Fusion](./images/wffusion89.png)

응답을 완료하는 데 시간이 조금 더 필요하다는 것을 알았으므로 이 HTTP 요청 앞에 타이머를 추가하여 즉시 실행되지 않도록 하는 것이 좋을 수 있습니다.

**도구** 노드를 클릭한 다음 **절전 모드**&#x200B;를 선택하십시오.

![WF Fusion](./images/wffusion90.png)

**여러 변수 설정**&#x200B;과(와) **Photoshop 검사 상태** 사이에 **절전 모드** 노드를 배치하십시오. **지연**&#x200B;을(를) **5**&#x200B;초로 설정하십시오. **확인**&#x200B;을 클릭합니다.

![WF Fusion](./images/wffusion91.png)

그럼 이걸로 드셔보세요 아래 구성의 문제는 5초의 기다림으로도 충분할 수 있지만 충분하지 않을 수 있다는 것입니다. 실제로는 상태가 **성공**&#x200B;이 될 때까지 5초마다 상태를 확인하는 do...while 루프와 같은 보다 지능적인 솔루션을 사용하는 것이 좋습니다. 이제 다음 단계에서 이러한 전술을 구현합니다.

![WF Fusion](./images/wffusion92.png)

**여러 변수 설정**&#x200B;과(와) **절전 모드** 사이의 **렌치** 아이콘을 클릭합니다. **모듈 추가**&#x200B;를 선택합니다.
...
**반복**&#x200B;을(를) **20**(으)로 설정합니다. **확인**&#x200B;을 클릭합니다.

![WF Fusion](./images/wffusion96.png)

그런 다음 **Photoshop 확인 상태**&#x200B;에서 **+**&#x200B;을(를) 클릭하여 다른 모듈을 추가합니다.
...
**Source 모듈**&#x200B;을(를) **반복**(으)로 설정합니다. **확인**&#x200B;을 클릭하세요.
...
**+ 항목 추가**&#x200B;를 클릭하고 **변수 이름**&#x200B;을(를) `done`(으)로 설정합니다.
...
이전에 구성한 **여러 변수 설정** 노드를 클릭합니다. **done** 변수를 초기화하려면 여기에서 `false`(으)로 설정해야 합니다. **+ 항목 추가**&#x200B;를 클릭합니다.

![WF Fusion](./images/wffusion107.png)

**변수 이름**&#x200B;의 경우 `done`을(를) 사용하십시오. 상태를 설정하려면 부울 값이 필요합니다. 부울 값을 찾으려면 **톱니바퀴** 아이콘을 클릭한 다음 `false`을(를) 선택하십시오. **추가를 클릭합니다**.
...
그런 다음 구성한 **여러 변수 가져오기** 노드 뒤에 있는 **렌치** 아이콘을 클릭합니다.
...
레이블은 **사용 완료되었습니까?** 질문에 답합니다. 이미 존재하는 변수 **완료**&#x200B;을(를) 사용하여 **조건**&#x200B;을(를) 설정합니다. 연산자는 **같음**(으)로 설정되어야 하고 값은 부울 변수 `false`이어야 합니다. **확인**&#x200B;을 클릭합니다.

![WF Fusion](./images/wffusion112.png)

그런 다음 노드 **Photoshop 검사 상태**&#x200B;와(과) **배열 집계** 사이에 공간을 만드십시오. 그런 다음 **렌치** 아이콘을 클릭하고 **라우터 추가**&#x200B;를 선택합니다. Photoshop 파일의 상태를 확인한 후 2개의 경로가 있어야 하므로 이 작업을 수행하는 것입니다. 상태가 `succeeded`이면 **완료**&#x200B;의 변수를 `true`(으)로 설정해야 합니다. 상태가 `succeeded`과(와) 같지 않으면 루프가 계속됩니다. 라우터에서 확인 및 설정을 할 수 있습니다.

![WF Fusion](./images/wffusion113.png)

라우터를 추가한 후 **렌치** 아이콘을 클릭하고 **필터 설정**&#x200B;을 선택합니다.

![WF Fusion](./images/wffusion114.png)

레이블은 **완료**&#x200B;를 사용합니다. 응답 필드 **data.outputs[].status**&#x200B;을(를) 선택하여 **Photoshop 검사 상태** 노드의 응답을 사용하여 **조건**&#x200B;을(를) 설정합니다. 연산자는 **같음**(으)로 설정되고 값은 `succeeded`이어야 합니다. **확인**&#x200B;을 클릭합니다.

![WF Fusion](./images/wffusion115.png)

그런 다음 물음표가 있는 빈 노드를 클릭하고 **도구**&#x200B;를 검색합니다. 그런 다음 **도구**&#x200B;를 선택합니다.
...
**변수 이름**&#x200B;의 경우 `done`을(를) 사용하십시오. **변수 값**&#x200B;의 경우 부울 값 `true`을(를) 사용해야 합니다. **톱니바퀴** 아이콘을 클릭한 다음 `true`을(를) 선택합니다. **추가를 클릭합니다**.
...
기존 변수를 제거하고 **+ 항목 추가**&#x200B;를 클릭합니다. **변수 이름**&#x200B;의 경우 `placeholder`을(를) 사용하고 **변수 값**&#x200B;의 경우 `end`을(를) 사용합니다. **추가**&#x200B;를 클릭한 다음 **확인**&#x200B;을 클릭합니다.

![WF Fusion](./images/wffusion123.png)

시나리오를 저장하려면 **저장**&#x200B;을 클릭하세요. 그런 다음 **한 번 실행**&#x200B;을 클릭합니다.

![WF Fusion](./images/wffusion124.png)

그런 다음 시나리오가 실행되며 성공적으로 완료됩니다. 구성한 do...while 루프가 제대로 작동한 것을 알 수 있습니다. 아래 실행에서는 **도구 > 여러 변수 가져오기** 노드의 버블을 기반으로 **반복**&#x200B;이 20번 실행되었음을 볼 수 있습니다. 해당 노드 후에 상태를 확인하는 필터를 구성했으며 상태가 **성공**&#x200B;과 같지 않은 경우에만 다음 노드가 실행되었습니다. 첫 번째 실행에서 상태가 이미 **성공**&#x200B;이었으므로 이 실행에서는 필터 뒤에 부분이 한 번만 실행되었습니다.
...
다음 단계: [1.2.3 Workfront Fusion을 사용한 프로세스 자동화](./ex3.md)
