...
- 데이터 연결 UI 이해
- Adobe Experience Platform 데이터를 CJA으로 가져오기
- 개인 ID 및 데이터 결합 이해
...
위에서 언급했듯이 각 데이터 세트에 대해 서로 다른 개인 ID를 설정할 수 있습니다. 이를 통해 CJA에서 여러 출처의 다양한 데이터 세트를 함께 가져올 수 있습니다. 컨텍스트와 어떤 일이 왜 일어났는지 이해하는 데 매우 흥미롭고 도움이 될 수 있는 NPS 또는 설문 조사 데이터를 가져온다고 상상해 보십시오.
...
[1.1.3(데이터 보기 만들기)으로 이동](./ex3.md){target="_blank"}
