...
컨텐츠 supply chain 자동화를 작성하는 맥락에서 특정 사용 사례에 대해 수행해야 하는 여러 파일 작업이 있는 경우가 많습니다. 예를 들어, 파일의 배경을 변경해야 하거나, 다양한 레이어의 텍스트를 변경해야 할 수도 있습니다. 모든 파일 작업을 동시에 수행할 수 있는 것은 아니므로 여러 단계로 진행해야 합니다. 모든 중간 단계 후 출력은 다음 단계를 실행하는 데 필요한 임시 파일입니다. 다음 단계가 실행되면 임시 파일이 빠르게 값을 잃고 더 이상 필요하지 않은 경우가 많으므로 이를 삭제해야 합니다.
...
>[!NOTE]
>
>이 튜토리얼을 방문 안내 워크숍 또는 안내 온디맨드 교육의 일부로 수행하는 경우 이미 Microsoft Azure 스토리지 계정에 액세스할 수 있습니다. 이 경우 자신의 계정을 만들 필요가 없습니다. 제공된 계정을 교육의 일부로 사용하십시오.

[https://portal.azure.com](https://portal.azure.com){target="_blank"}(으)로 이동하여 Azure 계정으로 로그인합니다. 계정이 없는 경우 개인 이메일 주소를 사용하여 Azure 계정을 만드십시오.
...
아직 구독하지 않은 경우 왼쪽 메뉴에서 **모든 리소스**&#x200B;를 선택하면 Azure 구독 화면이 나타납니다.

구독하지 않은 경우 **Azure 무료 평가판 시작**&#x200B;을 선택하세요.

![Azure 저장소](./images/04azurestartsubscribe.png){zoomable="yes"}

Azure 구독 양식을 작성하고 활성화를 위해 휴대폰과 신용 카드를 제공합니다(30일 동안 프리 티어가 제공되며 업그레이드하지 않는 한 요금이 부과되지 않음).
...
## 1.1.2.4 Azure 저장소 탐색기 설치

[파일을 관리하려면 Microsoft Azure 저장소 탐색기를 다운로드하십시오](https://azure.microsoft.com/en-us/products/storage/storage-explorer#Download-4){target="_blank"}. 특정 OS에 맞는 버전을 선택하고 다운로드하여 설치합니다.

![Azure 저장소](./images/az10.png){zoomable="yes"}

응용 프로그램을 열고 **Azure으로 로그인**&#x200B;을 선택합니다.
...
Microsoft Azure 저장소 탐색기 앱으로 돌아가서 구독을 선택하고 **탐색기 열기**&#x200B;를 선택합니다.
...
**SAS 토큰 URL**&#x200B;을(를) 사용하여 Azure 저장소 계정에 파일을 업로드하십시오.
...
다시 Azure 스토리지 탐색기로 돌아가면 폴더의 컨텐츠가 새로 고침되고 새로 업로드된 파일이 나타납니다.

![Azure 저장소](./images/az38.png){zoomable="yes"}

## 1.1.2.7 프로그래밍 파일 사용

장기적으로 Azure 저장소 계정에서 파일을 프로그래밍 방식으로 읽으려면 파일을 읽을 수 있는 권한을 가진 새 **SAS(공유 액세스 서명)** 토큰을 만들어야 합니다. 기술적으로 이전 연습에서 만든 SAS 토큰을 사용할 수 있지만 가장 좋은 방법은 **읽기** 권한만 있는 별도의 토큰과 **쓰기** 권한만 있는 별도의 토큰을 사용하는 것입니다.

### 장기 읽기 SAS 토큰

Azure 저장소 탐색기로 돌아가서 컨테이너를 마우스 오른쪽 단추로 클릭한 다음 **공유 액세스 서명 가져오기**&#x200B;를 선택합니다.
...
Azure 저장소 탐색기로 돌아가서 컨테이너를 마우스 오른쪽 단추로 클릭하고 **공유 액세스 서명 가져오기**&#x200B;를 선택합니다.
