...
여기에서 사용할 요청은 **비동기** 요청으로, 제출된 작업의 URL이 포함된 응답을 제공합니다. 즉, 작업 상태를 확인하고 생성된 이미지에 액세스하려면 두 번째 요청을 사용해야 합니다.

>[!NOTE]
>
>Firefly Image 4 및 Image 4 Ultra의 릴리스에서는 비동기 요청을 위해 동기 요청이 더 이상 사용되지 않습니다.
...
그러면 즉시 응답을 받게 됩니다. 이 응답에는 생성된 이미지의 이미지 URL이 포함되지 않고, 대신 시작한 작업의 상태 보고서 URL이 포함되어 있으며, 실행 중인 작업을 취소할 수 있는 다른 URL이 포함되어 있습니다.

>[!NOTE]
>
>사용 중인 Postman 컬렉션이 동적 변수를 사용하도록 구성되었습니다. 예를 들어, Postman에 구성된 **스크립트** 덕분에 필드 **statusUrl**&#x200B;이(가) Postman에서 동적 변수로 저장되었습니다.
...
>[!NOTE]
>
>사용 중인 Postman 컬렉션이 동적 변수를 사용하도록 구성되었습니다. 예를 들어 이전 요청의 **statusUrl** 필드는 Postman에서 동적 변수로 저장되었으며 이제 **GET - Firefly - 상태 보고서 가져오기** 요청에 대한 URL로 사용되고 있습니다.

![Firefly](./images/ff1b.png)

비슷한 응답을 받았어야 합니다. 실행된 작업의 개요입니다. 생성된 이미지가 포함된 필드 **url**&#x200B;을(를) 볼 수 있습니다. 응답에서 이미지 URL을 복사(또는 클릭)하고 웹 브라우저에서 열어 이미지를 봅니다.
...
요청 **POST - Firefly - T2I V3 비동기**&#x200B;의 **본문**&#x200B;에서 필드 `"promptBiasingLocaleCode": "en-US"` 아래에 다음을 추가하고 변수 `XXX`을(를) Firefly Services UI에서 임의로 사용한 시드 번호 중 하나로 바꿉니다. 이 예제에서 **seed** 숫자는 `142194`입니다.
...
**보내기**&#x200B;를 클릭합니다. 그러면 방금 제출한 작업의 상태 보고서에 대한 링크가 있는 응답을 다시 받게 됩니다.
...
그런 다음 요청의 **본문**&#x200B;에서 **POST - Firefly - T2I V3 비동기**&#x200B;을(를) **시드** 개체 아래에 아래 **스타일** 개체를 붙여 넣으십시오. 이렇게 하면 생성된 이미지의 스타일이 **art_deco**(으)로 변경됩니다.
...
그럼 이걸 드셔보세요 **보내기**&#x200B;를 클릭합니다. 그러면 방금 제출한 작업의 상태 보고서에 대한 링크가 있는 응답을 다시 받게 됩니다.
...
**POST - Firefly - T2I V3 비동기** 요청의 **Body**&#x200B;에서 **seeds** 개체에 대한 코드를 제거합니다. **보내기**&#x200B;를 클릭한 다음 응답에서 얻은 이미지 URL을 클릭합니다. 그러면 방금 제출한 작업의 상태 보고서에 대한 링크가 있는 응답을 다시 받게 됩니다.
...
**FF - Firefly Services Tech Insiders** 컬렉션에서 **POST - Firefly - Gen Expand async**(이)라는 요청을 선택하고 요청의 **Body**(으)로 이동합니다.
...
그러면 방금 제출한 작업의 상태 보고서에 대한 링크가 있는 응답을 다시 받게 됩니다.
...
**Firefly - T2I V3 비동기** 요청을 사용하여 새 이미지를 생성합니다.
...
배치 정렬을 변경하면 출력도 약간 달라집니다. 이 예제에서는 배치가 **left, bottom**(으)로 변경되었습니다. **보내기**&#x200B;를 클릭합니다. 그러면 방금 제출한 작업의 상태 보고서에 대한 링크가 있는 응답을 다시 받게 됩니다.
