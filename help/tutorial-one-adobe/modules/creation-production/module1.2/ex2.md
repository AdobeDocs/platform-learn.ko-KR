...
**저장소**&#x200B;에 대해 **Azure**&#x200B;을(를) 선택하고 `{{1.AZURE_STORAGE_URL}}/{{1.AZURE_STORAGE_CONTAINER}}/citisignal-fiber-replacedbg.psd{{1.AZURE_STORAGE_SAS_WRITE}}`파일 위치&#x200B;**에 이**&#x200B;을(를) 붙여 넣은 다음 **유형**&#x200B;에서 **vnd.adobe.photoshop**&#x200B;을(를) 선택합니다. **고급 설정 표시**&#x200B;를 사용하려면 클릭하세요.
...
이제 새 PSD 파일이 성공적으로 생성되어 Microsoft Azure 스토리지 계정에 저장되었음을 알 수 있습니다.
...
**입력 파일**&#x200B;의 경우 **입력 파일 저장소**&#x200B;에 대해 **Azure**&#x200B;을(를) 선택하고 이전 요청에서 출력을 선택하십시오. **Adobe Photoshop - PSD 편집 적용**. 이러한 요청은 다음과 같이 정의할 수 있습니다. ``{{XX.data[].`_links`.renditions[].href}}``(XX를 이전 모듈의 시퀀스 번호로 바꾸기 Adobe Photoshop - PSD 편집 적용).
...
**출력**&#x200B;이 표시될 때까지 아래로 스크롤합니다. **저장소**&#x200B;에 대해 **Azure**&#x200B;을(를) 선택하십시오. **파일 위치**&#x200B;에 대해 아래 위치를 입력하십시오. 생성된 모든 파일에 고유한 이름이 있는지 확인하는 데 사용되는 파일 이름에 `{{timestamp}}` 변수를 추가했는지 확인하십시오. 또한 **Type**&#x200B;을(를) **vnd.adobe.photoshop**(으)로 설정하십시오.
...
그런 다음 Azure 저장소 탐색기를 사용하여 Azure 저장소 탐색기에서 새로 만든 PSD 파일을 두 번 클릭하여 찾아 열 수 있습니다.
