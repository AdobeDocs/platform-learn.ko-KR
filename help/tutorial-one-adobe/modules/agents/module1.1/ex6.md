...
## 1.1.6.1 Agent Orchestrator 액세스

[https://ao.adobe.io/](https://ao.adobe.io/)&#x200B;(으)로 이동합니다. Adobe 계정을 사용하여 로그인합니다. 로그인 후 아래와 같이 인스턴스와 샌드박스를 선택하여 올바른 인스턴스와 샌드박스를 선택했는지 확인합니다.
...
그럼 이걸 보셔야죠 다음 명령을 입력하고 **보내기**&#x200B;를 클릭합니다.
...
## 1.1.6.3 전체 구매 트렌드로 시작하여 컨텍스트를 고정하고 파이버 확대
...
## 1.1.6.4에서 주문과 콘텐츠 환경 설정의 상관 관계를 지정합니다.
...
새 창이 열리면 즉시 여정 세부 사항 개요로 이동합니다.
...
## 1.1.6.7 폴아웃 분석을 통해 여정 성능의 유효성 검사
...
유사한 경우 이미 존재하는 대상을 이미 사용할 수 있으면 유사한 메시지가 표시되어야 합니다.

![Agent Orchestrator](./images/aotechlab32a.png)

계획을 검토합니다. **계획 승인**&#x200B;을 클릭합니다.
...
## 1.1.6.9 사용량이 많은 기존 대상을 찾아 사용 중인지 확인
...
## 1.1.6.10 파이버 최대 시작에 대한 새 여정 만들기
