...
텍스트 편집기로 돌아가서 **connect-distributed.properties** 파일로 이동한 다음 마지막 줄(스크린샷의 89행)까지 아래로 스크롤합니다. `#`(으)로 시작하는 줄의 주석 처리를 제거하고 `# plugin.path=` 폴더의 전체 경로를 붙여 넣어야 합니다. `connectors` 결과는 다음과 유사해야 합니다.
...
그런 다음 전체 경험 이벤트를 클립보드에 복사합니다. JSON 페이로드의 공백을 제거해야 하며 이를 위해 온라인 도구를 사용합니다. [https://jsonviewer.stack.hu/](https://jsonviewer.stack.hu/)&#x200B;(으)로 이동하여 이 작업을 수행합니다.
...
>[!NOTE]
>
>콜 센터 상호 작용을 프로필 뷰어 패널에 표시하려면 [프로필 뷰어](https://dsn.adobe.com) 탭으로 이동하여 **이벤트** 아래에 새 줄을 추가하고 다음 변수를 사용하여 **https://dsn.adobe.com**&#x200B;에서 프로젝트에 아래 레이블과 필터를 추가해야 합니다.
