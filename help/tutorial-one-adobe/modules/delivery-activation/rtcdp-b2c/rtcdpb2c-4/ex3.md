---
title: Audience Activation - Microsoft Azure 이벤트 허브 - Adobe Experience Platform에서 이벤트 허브 RTCDP 대상 설정
description: Audience Activation - Microsoft Azure 이벤트 허브 - Adobe Experience Platform에서 이벤트 허브 RTCDP 대상 설정
kt: 5342
doc-type: tutorial
...
이전 연습에서 Event Hub 및 EventHub 네임스페이스를 정의했습니다. [Azure에서 Event Hub 설정](./ex2.md)

### 이벤트 허브 네임스페이스

Azure 포털에서 위의 정보를 조회하려면 [https://portal.azure.com/#home](https://portal.azure.com/#home)&#x200B;(으)로 이동합니다. 올바른 Azure 계정을 사용하고 있는지 확인하십시오.
...
이 시점에서 Adobe Experience Platform Real-time CDP에서 Azure 이벤트 허브 대상을 정의하는 데 필요한 모든 값을 식별했어야 합니다.
...
| namespace | 이벤트 허브 네임스페이스 | `--aepUserLdap---aep-enablement` |
| 이벤트 허브 이름 | 이벤트 허브 | `--aepUserLdap---aep-enablement-event-hub` |

## Adobe Experience Platform에서 Azure 이벤트 허브 대상 만들기
...
**대상**(으)로 이동한 다음 **카탈로그**(으)로 이동합니다. **클라우드 저장소**&#x200B;를 선택하고 **Azure 이벤트 허브**(으)로 이동한 다음 **설정**&#x200B;을 클릭합니다.
