...
1. **[!UICONTROL 경로가]**&#x200B;인 경우 `/` `OR` `/index.html`을(를) 설정합니다. Luma 데모 웹 사이트의 경우 규칙이 홈 페이지에서만 트리거되도록 합니다
...
1. [Luma 데모 웹 사이트](https://luma.enablementadobe.com){target="_blank"}(으)로 이동
...
위에서 [Luma 홈 페이지](https://luma.enablementadobe.com)에 대한 데이터 스트림 재정의를 구성했습니다.  이 구성의 유효성을 검사하려면
...
[Livingston 다목적 타이트 제품 페이지](https://luma.enablementadobe.com/product.html?id=LLMP09)와 같은 제품 페이지로 이동합니다.  Analytics에서 컨텐츠 페이지 보기를 캡처하는지 확인합니다.
...
1. 장바구니에 [마르코 경량 활성 후드](https://luma.enablementadobe.com/product.html?id=LLMH13) 추가
1. [장바구니 페이지](https://luma.enablementadobe.com/cart.html)&#x200B;(으)로 이동하여 다음 항목에 대한 Edge 추적 확인
