...
1. 태그 규칙 `all pages - adobeDataLayer push - set global variables - 1`을(를) 엽니다
1. 새 **[!UICONTROL 작업]** 추가
...
1. `eVar1`을(를) `Page Name` 데이터 요소로 설정
1. `prop1`을(를) 설정하여 `eVar1`의 값을 복사합니다.
1. `data` 개체가 XDM 매핑 값을 덮어쓰는 방법을 보여 주기 위해 **[!UICONTROL 추가 속성]** 섹션에서 페이지 이름을 정적 값 `test`(으)로 설정합니다.
...
1. 태그 규칙 `all pages - adobeDataLayer push - send event - 50`을(를) 엽니다
...
방문자가 특정 페이지에 있을 때 사용되는 Adobe Analytics 보고서 세트를 변경할 수 있습니다. 이를 위해서는 데이터 스트림과 규칙 모두에서 구성해야 합니다.
...
1. 새 규칙을 만들어 이름을 `homepage - adobeDataLayer push - AA report suite override - 51`로 지정합니다.
...
1. **[!UICONTROL 고급 옵션]**&#x200B;을(를) 열려면 선택하고 `51`을(를) 입력하십시오. 이렇게 하면 `all pages - adobeDataLayer push - send event - 50`변수 업데이트&#x200B;**[!UICONTROL 작업 유형으로 기준 XDM을 설정하는]** 이후에 규칙이 실행됩니다.
1. **[!UICONTROL 변경 내용 유지]** 선택
...
1. **[!UICONTROL 경로가]**&#x200B;인 경우 `/` `OR` `/index.html`을(를) 설정합니다. Luma 데모 사이트의 경우 규칙이 홈 페이지에서만 트리거되도록 합니다
...
1. **[!UICONTROL XDM 데이터]**(으)로, `XDM Variable`데이터 요소 만들기[&#x200B; 단원에서 만든 &#x200B;](create-data-elements.md) 데이터 요소를 선택합니다
...
1. [Luma 데모 사이트](https://newluma.enablementadobe.com){target="_blank"}(으)로 이동
...
1. Luma 사이트에서:
   ...
   >[!NOTE]
   >
   >로그인했으므로 `f660ab912ec121d1b1e928a0bb4bc61b`에서 **`test@test.com`** 사용자에 대한 인증된 ID `[!UICONTROL c.a.x.identitymap.lumacrmid.[0].id]`의 유효성을 검사하는 동안 잠시 기다려 주십시오.

### 보고서 세트 재정의 유효성 검사

위에서 [Luma 홈 페이지](https://newluma.enablementadobe.com)에 대한 데이터 스트림 재정의를 구성했습니다.  이 구성의 유효성을 검사하려면
...
[Livingston 다목적 타이트 제품 페이지](https://newluma.enablementadobe.com/product.html?id=LLMP09)와 같은 제품 페이지로 이동합니다.  Analytics에서 컨텐츠 페이지 보기를 캡처하는지 확인합니다.
...
1. 제품 페이지에 있으므로 `[!UICONTROL c.a.x.eventType]`이(가) `commerce.productViews`(으)로 설정되어 있습니다.
   ...
   >[!TIP]
   >
   > `product detail pages - adobeDataLayer push - set product details variables - 20` 규칙은 `eventType` 규칙이 설정한 `all pages - adobeDataLayer push - set global variables - 1`의 값을 시퀀스 뒷부분에서 트리거하도록 설정했으므로 덮어씁니다
...
1. 장바구니에 [마르코 경량 활성 후드](https://newluma.enablementadobe.com/product.html?id=LLMH13) 추가
1. [장바구니 페이지](https://newluma.enablementadobe.com/cart.html)&#x200B;(으)로 이동하여 다음 항목에 대한 Edge 추적 확인
...
장바구니 및 구매 이벤트를 검토하여 구현의 유효성을 계속 확인합니다.
...
>[!TIP]
>
> 이 단원을 완료한 후에는 `homepage - adobeDataLayer push - AA report suite override - 51` 규칙을 사용하지 않도록 설정하는 것이 좋습니다.
