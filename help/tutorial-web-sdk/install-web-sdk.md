...
태그 속성은 웹 페이지에서 세부 정보를 수집하여 여러 위치로 보내는 데 필요한 모든 JavaScript, 규칙 및 기타 기능을 위한 컨테이너입니다.
...
1. **[!UICONTROL 이름]**(으)로 `Luma Web SDK Tutorial`을(를) 입력하십시오(회사에서 여러 사람이 이 자습서를 수강하는 경우 마지막에 이름을 추가하십시오.)
...
1. 사이드바가 열리도록 `Adobe Experience Platform Web SDK` 확장을 선택하십시오.
1. **[!UICONTROL 설치]** 선택
...
대부분의 기본 설정을 그대로 두고 필요에 따라 나중에 업데이트합니다. 이제 해야 하는 유일한 작업은 확장을 데이터 스트림에 연결하는 것입니다. 이렇게 하면 브라우저에서 수행한 네트워크 호출에 올바른 데이터 스트림/구성 ID가 추가되므로, 호출이 Platform Edge Network에 도달하면 올바른 데이터 스트림으로 라우팅됩니다.

1. **[!UICONTROL 데이터스트림]**&#x200B;에서 **[!UICONTROL 목록에서 선택]** 입력 방법을 선택합니다.
1. **[!UICONTROL 프로덕션 환경]**&#x200B;을 구성하십시오. (속성의 다른 태그 환경은 프로덕션 설정을 상속합니다.)
   1. 스키마, ID 네임스페이스 및 데이터스트림을 생성한 샌드박스를 선택합니다
   1. 이전에 만든 데이터 스트림 `Luma Web SDK: Development Environment`을(를) 선택하십시오.
1. **[!UICONTROL 저장]** 선택
