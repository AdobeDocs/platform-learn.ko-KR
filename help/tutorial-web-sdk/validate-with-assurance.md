...
[Adobe Experience Platform Assurance](https://experienceleague.adobe.com/ko/docs/experience-platform/assurance/home)은(는) 데이터를 수집하거나 경험을 제공하는 방법을 검사, 증명, 시뮬레이션 및 확인하는 데 도움이 되는 기능입니다.

[데이터 스트림 구성](configure-datastream.md) 단원에서 학습한 대로 Platform Web SDK은 먼저 디지털 속성에서 Platform Edge Network으로 데이터를 전송합니다. 그런 다음 Platform Edge Network이 데이터를 데이터스트림에 활성화된 서비스로 전달합니다. Assurance을 사용하여 Platform Edge Network에서 들어오고 나가는 요청의 유효성을 검사할 수 있습니다.
...
1. **[!UICONTROL Edge]** 탭을 선택하고 **[!UICONTROL 연결]**&#x200B;을 선택합니다.
   ...
   ![Edge 추적 연결](assets/assurance-edgeTrace-connect.png)

1. 지금은 비어 있습니다

   ![연결된 Edge 추적](assets/analytics-debugger-edge-connected.png)

1. [Luma 홈 페이지](https://luma.enablementadobe.com/)를 새로 고치고 **[!UICONTROL Experience Platform Debugger]**&#x200B;를 다시 확인하여 데이터가 Platform Edge Network으로 들어오는 것을 확인합니다. 향후 단원에서는 데이터 스트림에서 서비스를 활성화할 때 나가는 요청을 볼 수 있습니다.

   Edge 추적의 ![요청](assets/validate-edge-trace.png)

   Adobe Experience Platform Debugger에서 Edge 추적을 활성화할 때마다 Assurance 세션이 백그라운드에서 시작됩니다. 여기에서 정보를 검토할 수 있지만 Assurance 인터페이스가 훨씬 더 유용할 것입니다.
...
아직 데이터 스트림에서 서비스를 활성화하지 않았기 때문에 이 구현 단계에서 볼 수 있는 정보가 제한됩니다.

### `Alloy Request`(으)로 웹 SDK에서 들어오는 요청 보기

Web SDK에서 들어오는 히트가 에지에 수신되면 볼 수 있습니다.

1. `Alloy Request` 행 선택
1. 익숙한 변수가 있는 XDM 개체를 찾을 때까지 원시 이벤트를 살펴보거나 [!UICONTROL 페이로드] > `ACPExtensionEventData`에서 노드를 확장하십시오.

   ![Alloy 요청](assets/assurance-alloy-request.png)


### `Alloy Response Handle`에서 응답 보기

아시다시피 Experience Cloud ID(ECID)는 Platform Edge Network에서 생성된 후 웹 SDK 응답에 표시됩니다. Assurance에서 볼 수 있는 응답에서 살펴보겠습니다.

1. 이벤트가 `Alloy Response Handle`인 행을 필터링하고 선택하십시오.
...
1. `[!UICONTROL payload > 0 > payload > 0 > namespace]`을(를) 선택하여 드릴다운합니다. 마지막 `0`에 표시된 ID는 `ECID`에 해당합니다. `namespace`과(와) 일치하는 `ECID`에 표시되는 값 기준
   ...
   ![Assurance Alloy 응답](assets/assurance-alloy-response.png)
