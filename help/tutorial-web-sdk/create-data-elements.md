...
[Luma 데모 사이트](https://newluma.enablementadobe.com)에서 컨텐츠, 상거래 및 ID 데이터의 태그에 데이터 요소를 만드는 방법을 알아봅니다. 그런 다음 XDM 스키마의 필드를 Adobe Experience Platform 웹 SDK 확장 변수 데이터 요소 유형으로 채웁니다.
...
>[!IMPORTANT]
>
>이 단원의 데이터는 Luma 사이트의 `[!UICONTROL adobeDataLayer]` 데이터 레이어에서 가져옵니다. 데이터 레이어를 보려면 개발자 콘솔을 열고 `[!UICONTROL adobeDataLayer]`을(를) 입력하여 사용 가능한 전체 데이터 레이어를 확인하십시오.![adobeDataLayer 데이터 레이어](assets/data-element-data-layer-new.png)
...
XDM 개체를 만들기 전에 [Luma 데모 사이트](https://newluma.enablementadobe.com){target="_blank"} 데이터 레이어에 대해 다음 데이터 요소 집합을 만드십시오.
...
* 다음 사용자 지정 코드를 사용하는 **`Ecommerce Checkout Products`**:
