...
이 튜토리얼에서는 샘플 소매 웹 사이트인 Luma에서 Platform Web SDK을 구현하는 과정을 안내합니다. [Luma 사이트](https://newluma.enablementadobe.com)에는 사실적인 구현을 만들 수 있는 풍부한 데이터 레이어와 기능이 있습니다. 이 자습서에서는 다음 작업을 수행합니다.

* Luma 웹 사이트에 대한 Platform Web SDK 구현을 사용하여 자체 계정에서 고유한 태그 속성을 만듭니다.
* 데이터스트림, 스키마 및 ID 네임스페이스 등 웹 SDK 구현에 사용되는 주요 데이터 수집 기능을 구성합니다.
...
>[!NOTE]
>
>[모바일 SDK](../tutorial-mobile-sdk/overview.md)에서도 유사한 자습서를 사용할 수 있습니다.
...
이 단원에서는 Adobe 사용자 계정과 단원을 완료하는 데 필요한 권한이 있다고 가정합니다. 그렇지 않은 경우 회사의 Experience Cloud 관리자에게 연락하여 액세스 권한을 받아야 합니다.
...
* 선택적 **Journey Optimizer** 단원의 경우 **[!UICONTROL 캠페인 관리]**, **[!UICONTROL 캠페인 게시]** 및 **[!UICONTROL 캠페인 보고서 보기]**&#x200B;에 대한 권한 항목이 있어야 합니다.
  <!--
  ...
  * See the documentation for more information on [Journey Optimizer permission configuration](https://experienceleague.adobe.com/ko/docs/journey-optimizer/using/access-control/high-low-permissions#campaign-capability).
  -->
...
* 2026년 2월 27일: 이벤트 기반 데이터 레이어가 포함된 새로운 Luma 웹 사이트
...
자습서 중에 필요할 때마다 쉽게 로드할 수 있도록 별도의 브라우저 탭에서 [Luma 웹 사이트](https://newluma.enablementadobe.com){target="blank"}를 로드하고 책갈피로 지정합니다. 호스팅된 프로덕션 사이트를 로드할 수 있는 것 외에 Luma에 대한 추가 액세스 권한이 필요하지 않습니다.

[![Luma 웹 사이트](assets/overview-luma.png)](https://newluma.enablementadobe.com){target="blank"}

시작해 보겠습니다! 다음: [웹 데이터에 대한 XDM 스키마 만들기](configure-schemas.md)
