...
데이터 수집 태그 및 [Luma 데모 사이트](https://newluma.enablementadobe.com)에 익숙하고 자습서의 이전 단원을 완료했습니다.
...
Luma 웹 사이트에서는 Adobe 클라이언트 데이터 레이어(ACDL)라는 이벤트 기반 데이터 레이어를 사용합니다. 이벤트가 발생할 때마다 `adobeDataLayer` 배열로 푸시됩니다. 많은 기본 옵션이지만 이러한 이벤트를 사용하여 규칙을 구성할 것입니다.
...
태그에서 규칙은 다양한 조건에서 작업(호출 실행)을 실행하는 데 사용됩니다. Platform Web SDK 태그 확장에는 규칙에 사용되는 두 가지 작업이 포함되어 있습니다.

* **[!UICONTROL 변수 업데이트]** 데이터 요소를 XDM 또는 데이터 변수에 매핑합니다.
* **[!UICONTROL 이벤트 보내기]**&#x200B;에서 Experience Platform Edge Network으로 데이터를 보냅니다.

이 단원의 나머지 부분에서

1. **[!UICONTROL 변수 업데이트]** 작업을 사용하여 XDM 필드의 &quot;전역 구성&quot;을 정의합니다.

1. 특정 조건(예: 제품 페이지에 제품 세부 사항 추가)에서 &quot;전역 구성&quot;을 무시하고 추가 XDM 필드를 제공하는 **[!UICONTROL 변수 업데이트]** 작업을 사용하십시오.

1. **[!UICONTROL 이벤트 보내기]** 작업을 사용하여 원하는 모든 데이터를 Adobe Experience Platform Edge Network으로 보냅니다.

이러한 모든 규칙은 &quot;[!UICONTROL order]&quot; 옵션을 사용하여 올바르게 순서가 지정됩니다.

이 비디오에서는 프로세스에 대한 개요를 제공합니다.

>[!VIDEO](https://video.tv.adobe.com/v/3454034/?captions=kor&learn=on&enablevpops)
...
1. 이제 XDM 필드를 적절한 값에 매핑하여 지정합니다.
   ...
   >[!TIP]
   >
   > 데이터 요소가 null인 경우 XDM 필드는 네트워크 요청에 포함되지 않습니다. 따라서 사용자가 인증되지 않았고 `Identity Map` 데이터 요소가 null이면 `identityMap` 개체가 전송되지 않습니다. 이것이 &quot;전역 구성&quot;에서 이 구성을 정의할 수 있는 이유입니다.
...
1. **[!UICONTROL 변경 내용 유지]**&#x200B;를 선택한 다음 다음 다음 화면에서 **[!UICONTROL 저장]**&#x200B;을 선택하여 규칙을 완료합니다
...
1. 이러한 XDM 필드를 적절한 값에 매핑:
   ...
   | `productListItems.name` | `Ecommerce Product Name`(**[!UICONTROL 개별 항목 제공]** 및 **[!UICONTROL 항목 추가]**&#x200B;를 먼저 선택) |
