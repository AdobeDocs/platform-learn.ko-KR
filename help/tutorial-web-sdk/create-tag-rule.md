...
* **[!UICONTROL 이벤트 보내기]**&#x200B;에서 Experience Platform Edge Network에 XDM 개체를 보냅니다.
...
1. **[!UICONTROL 고급]** 드롭다운을 선택하고 **[!UICONTROL 순서]**(으)로 `1`을(를) 입력하십시오.
...
1. **[!UICONTROL 데이터 요소]**(으)로 [데이터 요소 만들기](create-data-elements.md) 단원에서 만든 `xdm.variable.content`을(를) 선택합니다.
...
1. **[!UICONTROL 고급 옵션]**&#x200B;을(를) 열려면 선택하고 `20`을(를) 입력하십시오. 이 순서 값은 전역 구성을 설정하는 `all pages - library loaded - set global variables - 1`의 _after_ 규칙이 실행되도록 합니다.
...
변수를 설정했으므로 **[!UICONTROL Edge Network 보내기]** 작업으로 전체 XDM 개체를 Platform 이벤트에 보내는 규칙을 만들 수 있습니다.
