...
>[!NOTE]
> 
>이 자습서에서는 데모용으로 [클라로](https://klaro.org/) CMP로서 Klaro 또는 웹 사이트에 사용하는 CMP를 따르십시오.
...
1. 방문 [클라로](https://klaro.org/) 계정을 설정합니다.
1. 다음으로 이동 **개인 정보 관리자** 지침에 따라 인스턴스를 생성합니다.
1. 사용 **통합 코드** 를 사용하여 Klaro를 태그 속성에 삽입합니다(지침은 다음 연습에 나와 있습니다).
1. 건너뛰기 **스캔 중** 섹션에서, Luma 데모 웹 사이트에서 하드코딩된 태그 속성을 감지하므로 이 자습서를 위해 빌드한 속성이 아닙니다.
...
에 대한 자세한 내용은 [!UICONTROL 동의 설정] 작업, 참조 [동의 설정](https://experienceleague.adobe.com/en/docs/experience-platform/tags/extensions/client/web-sdk/action-types#set-consent).
