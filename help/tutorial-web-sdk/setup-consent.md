...
1. 구성 활성화
   ...
   >[!NOTE]
   > 
   >일반적으로 위의 단계는 OneTrust 또는 TrustArc와 같은 CMP를 처리할 책임이 있는 팀이나 개인이 수행하고 처리합니다.
...
1. Experience Platform Web SDK 태그 확장 기능의 **[!UICONTROL 동의]** 섹션에서 **[!UICONTROL 기본 동의]**&#x200B;이(가) **[!UICONTROL In]**(으)로 설정되어 있는지 확인하십시오.
...
3. Luma 데모 웹 사이트에서 태그 라이브러리를 로드합니다
4. Luma 사이트에서 태그 디버깅을 활성화하고 페이지를 다시 로드합니다. 브라우저의 개발자 콘솔에서 defaultConsent가 **[!UICONTROL In]**&#x200B;과(와) 같음을 확인해야 합니다.
5. 이 구성을 사용하면 방문자가 쿠키 및 옵트아웃을 거부하기로 결정할 때까지 Experience Platform Web SDK 확장은 Platform Edge Network에 네트워크 요청을 수행합니다.
...
1. **[!UICONTROL Action]**(으)로, Experience Platform Web SDK 확장 [!UICONTROL 동의 설정] [!UICONTROL 작업 유형]을(를) 사용하여 동의를 &quot;out&quot;으로 설정합니다.

   ![동의 규칙 옵트아웃 작업](assets/consent-rule-optout-action.png)

1. 라이브러리를 저장하고 다시 빌드합니다.


이제 방문자가 옵트아웃하면 위의 방식으로 구성된 규칙이 실행되고 웹 SDK 동의를 **[!UICONTROL Out]**(으)로 설정합니다.

Luma 데모 웹 사이트로 이동하여 유효성을 검사하고, 쿠키를 거부하며, 옵트아웃 후 웹 SDK 요청이 실행되지 않는지 확인합니다.
...
1. Experience Platform Web SDK 확장의 **[!UICONTROL 동의]** 섹션에서 필요에 따라 기본 동의를 **[!UICONTROL 종료]** 또는 **[!UICONTROL 보류 중]**(으)로 설정합니다.

1. 태그 라이브러리에 업데이트된 구성을 **저장**&#x200B;하고 다시 빌드합니다.

   ![AEP 확장 동의 구성](assets/consent-implied-opt-out.png)
...
>[!TIP]
>
> 이 단원을 완료한 후에는 세 가지 새 규칙을 비활성화하는 것이 좋습니다.
