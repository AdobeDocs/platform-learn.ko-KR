...
>[!NOTE]
>
>이 자습서에서는 태그 및 플리커 완화 기능이 비동기적으로 구현된 [Luma 웹 사이트](https://newluma.enablementadobe.com){target=_blank}를 사용합니다. 이 섹션은 Platform Web SDK에서 플리커 완화가 작동하는 방식을 이해하는 데 참조됩니다.
...
1. **[!UICONTROL 사전 숨김 스타일 편집]** 단추 선택
   ...
   ![동기 구현을 위해 코드 조각 사전 숨김을 타깃팅합니다](assets/target-flicker-sync.png)
...
1. `all pages - adobeDataLayer push - send event - 50` 규칙 열기
...
1. 활동 URL에 대한 Luma 홈 페이지 및 Target 작업 공간(있는 경우)을 사용하여 경험 타깃팅(XT) 활동을 만듭니다.
...
1. 활동 이름을 업데이트한 다음 **[!UICONTROL 다음]**&#x200B;을 선택합니다.
...
1. [Luma 데모 사이트](https://newluma.enablementadobe.com)&#x200B;(으)로 이동하여 디버거를 사용하여 [사이트의 태그 속성을 자신의 개발 속성으로 전환](validate-with-debugger.md#use-the-experience-platform-debugger-to-map-to-your-tags-property)합니다.
1. 페이지 다시 로드
...
1. 규칙을 Luma 홈 페이지로 제한하는 조건을 추가합니다(쿼리 문자열이 없는 경로는 `/` 또는 `/index.html`과(와) 같음).
...
1. 다음 코드를 콘텐츠 상자에 붙여 넣어 영웅의 call-to-action을 변경합니다.
...
XDM 오브젝트에서 매핑되지 않은 Target에 유용한 매개 변수가 있습니다. 이러한 특수 Target 매개 변수는 다음과 같습니다.
...
1. 태그 규칙 `product detail pages - adobeDataLayer push - set product details variables - 20`을(를) 엽니다
1. 새 **[!UICONTROL 작업]** 추가
...
1. **[!UICONTROL 대상]** 개체 선택
1. `entity.id`을(를) `Ecommerce Product Id` 데이터 요소로 설정
1. `entity.name`을(를) `Ecommerce Product Name` 데이터 요소로 설정
1. `user.categoryId`을(를) `Ecommerce Product Category` 데이터 요소로 설정
1. **[!UICONTROL 변경 내용 유지]** 선택
1. **[!UICONTROL 저장]** 선택
...
1. `all pages - adobeDataLayer push - send event - 50` 규칙 열기
1. `Adobe Experience Platform Web SDK - Send event` 액션 선택
1. 아직 추가되지 않은 경우 데이터 필드에 `Data Variable` 데이터 요소를 추가합니다.
...
Luma 사이트에서는 데이터 레이어가 빠르게 로드되고 Send Event 호출이 실행되기 전에 완전히 정의됩니다. 이를 통해 단일 호출을 사용하여 개인화된 콘텐츠(예: Adobe Target에서)를 가져오고 분석 데이터(예: Adobe Analytics으로)를 전송할 수 있습니다.
...
두 번째 호출은 개인화된 콘텐츠의 모든 보기를 보고하고 `decisioning.propositionDisplay` 호출을 대체합니다.
...
1. `all pages - window loaded - send event - 50`(이)라는 새 규칙 만들기
1. 규칙에 이벤트를 추가합니다. **Core** 확장 및 **[!UICONTROL Window Loaded]** 이벤트 유형 사용
...
1. 이렇게 하면 선택한 **[!UICONTROL 렌더링된 제안 포함]** 확인란이 잠기므로 결정 요청에서 대기 중인 디스플레이 알림이 전송됩니다.
1. **[!UICONTROL 변경 내용 유지]** 선택
1. 규칙을 **[!UICONTROL 저장]**
1. 라이브러리 업데이트 및 다시 빌드

![send_decision_request_alone](assets/target-aa-request-guided.png)

>[!TIP]
>
>을(를) 사용하여 Decisioning 제안을 가져오는 이벤트에 뒤에 Adobe Analytics 이벤트가 없는 경우 **[!UICONTROL 안내 이벤트 사용]**&#x200B;을 선택하지 않은 상태로 둡니다. 모든 옵션을 수동으로 선택해야 하지만 가져오기 요청과 함께 **[!UICONTROL 렌더링된 제안을 포함]**&#x200B;하는 옵션의 잠금이 해제됩니다.
...
1. [Luma 데모 사이트](https://newluma.enablementadobe.com)&#x200B;(으)로 이동
1. 제품 세부 사항 페이지로 이동
