...
>[!TIP]
>
>프로필이 표시되지 않으면 [!UICONTROL 데이터 세트] 모든 데이터 세트가 성공적으로 생성되고 수집된 데이터를 확인하는 페이지입니다. 정상적으로 보이려면 15분 정도 기다렸다가 뷰어에서 프로필을 사용할 수 있는지 확인하십시오.  데이터 처리에 문제가 있는 경우 오류 메시지를 확인하여 문제를 찾으십시오. 또한 [!UICONTROL 데이터 세트] 페이지를 열고 json 데이터 파일을 드래그 앤 드롭하여 데이터를 다시 섭취합니다.
