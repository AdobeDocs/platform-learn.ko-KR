...
1. 을 선택하여 스키마 이름 지정 **[!UICONTROL 제목 없는 스키마]** 왼쪽 상단에서 **[!UICONTROL 표시 이름]** 및 **[!UICONTROL 설명]**, 예 `Luma Mobile App Event Schema` 및 `Schema for Luma mobile app experience events.`
...
1. 다음을 제공합니다. **[!UICONTROL 표시 이름]** 및 **[!UICONTROL 설명]**, 예 `App Information` 및 `Custom data type describing "Screen Views" & "App Actions"`
   ...
   ![이름 및 설명 입력](assets/schema-datatype-name.png)
...
1. 필드를 추가하려면 ![플러스](https://spectrum.adobe.com/static/icons/workflow_18/Smock_AddCircle_18_N.svg) 단추를 클릭합니다.
   ...
   이 필드는 앱 상호 작용을 위한 컨테이너 개체입니다.

1. 낙타 상자 제공 **[!UICONTROL 필드 이름]** `appInteraction`, **[!UICONTROL 표시 이름]** `App Interaction`, 및 선택 `Object` 다음에서 **[!UICONTROL 유형]** 목록을 표시합니다.

1. **[!UICONTROL 적용]**&#x200B;을 선택합니다.

   ![새 앱 작업 이벤트 추가](assets/schema-datatype-app-action.png)

1. 작업이 발생한 빈도를 측정하려면 다음을 선택하여 필드를 추가합니다. ![플러스](https://spectrum.adobe.com/static/icons/workflow_18/Smock_AddCircle_18_N.svg) 단추 옆에 있는 **[!UICONTROL appInteraction]** 개체를 만들었습니다.
...
1. 을(를) 만들려면 `appStateDetails` 이 포함된 개체 **[!UICONTROL 측정]** 필드 호출됨 `screenView` 및 2 **[!UICONTROL 문자열]** 필드 호출됨 `screenName` 및 `screenType` 을(를) 만들 때와 동일한 단계를 수행합니다 **[!UICONTROL appInteraction]** 개체.
...
1. 다음을 선택하여 사용자 정의 필드 그룹 만들기 **[!UICONTROL 새 필드 그룹 만들기]** 상단 근처에 라디오 버튼이 있습니다.

1. 다음을 제공합니다. **[!UICONTROL 표시 이름]** 및 **[!UICONTROL 설명]**, 예: `App Interactions` 및 `Fields for app interactions`.

1. **필드 그룹 추가**&#x200B;를 선택합니다.

   ![이름 및 설명 입력](assets/schema-fieldgroup-name.png)
...
1. 선택 **[!UICONTROL 앱 상호 작용]** 다음에서 **[!UICONTROL 유형]** 드롭다운은 이전 연습에서 만든 데이터 유형입니다.
...
>[!NOTE]
>
>사용자 정의 필드 그룹은 항상 Experience Cloud 조직 식별자 아래에 배치됩니다. So `_techmarketingdemos` 는 조직의 고유 값으로 대체됩니다.
