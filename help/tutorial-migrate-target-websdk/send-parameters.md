---
title: 매개 변수 보내기 - Target을 at.js 2.x에서 Web SDK으로 마이그레이션
...
# Platform Web SDK을 사용하여 Target에 매개 변수 보내기
...
이러한 페이지에 대한 Target 매개 변수는 Platform 웹 SDK을 사용하여 다르게 전송됩니다. at.js를 사용하여 Target에 매개 변수를 전달하는 방법에는 여러 가지가 있습니다.
...
Platform Web SDK은 추가 기능 없이 데이터를 일관성 있게 보낼 수 있는 단일 방법을 제공합니다. 모든 매개 변수는 `sendEvent` 명령을 사용하여 페이로드에서 전달되어야 하며 다음 두 가지 범주에 속합니다.

- `xdm` 개체에서 자동으로 매핑됨
- `data.__adobe.target` 개체를 사용하여 수동으로 전달됨

아래 표에서는 Platform Web SDK을 사용하여 예제 매개 변수가 다시 매핑되는 방법을 설명합니다.
...
Platform Web SDK `data.__adobe.target` 명령 페이로드의 `sendEvent` 개체 아래에 대상 프로필 매개 변수를 전달해야 합니다.
...
엔티티 매개 변수는 Target Recommendations에 대한 동작 데이터 및 보충 카탈로그 정보를 전달하는 데 사용됩니다. at.js에서 지원하는 모든 [엔티티 매개 변수](https://experienceleague.adobe.com/docs/target/using/recommendations/entities/entity-attributes.html?lang=ko)도 Platform Web SDK에서 지원됩니다. 프로필 매개 변수와 유사한 모든 엔터티 매개 변수는 Platform Web SDK `data.__adobe.target` 명령 페이로드의 `sendEvent` 개체 아래에 전달되어야 합니다.
...
1. `sendEvent` 필드 그룹을 사용하여 `identityMap` 명령 실행
...
>[!NOTE]
>
> Adobe에서는 인증된 id와 같이 개인을 나타내는 네임스페이스를 기본 id로 보낼 것을 권장합니다.



## Platform Web SDK 예제

Platform Web SDK을 사용하여 다양한 Target 매개 변수가 매핑되는 방법을 이해했으므로 두 예제 페이지를 아래와 같이 at.js에서 Platform Web SDK으로 마이그레이션할 수 있습니다. 예제 페이지는 다음과 같습니다.
