---
title: at.js 2.x와 Web SDK 비교 - Target을 at.js 2.x에서 Web SDK으로 마이그레이션
...
독립형 Adobe Target at.js 라이브러리는 Platform Web SDK과 크게 다릅니다. 다음 표는 마이그레이션 프로세스 중에 초점을 두어야 할 구현 영역을 평가하는 데 도움이 되는 참조입니다.
...
Platform Web SDK을 처음 사용하는 경우 걱정하지 마십시오. 아래 항목은 이 자습서 전체에서 자세히 다룹니다.
...
| Mbox 타사 ID | 지원됨 | 지원됨 |
| 고객 속성 | 지원됨 | 지원됨 |
| 원격 오퍼 | 지원됨 | 부분적으로 지원됨. 다이내믹 원격 오퍼는 지원되지 않습니다. |
| 리디렉션 오퍼 | 지원됨 | 지원됨. 그러나 Platform Web SDK이 있는 페이지에서 at.js가 있는 페이지(및 반대 방향)로의 리디렉션은 지원되지 않습니다. |
| 온디바이스 의사 결정 | 지원됨 | 현재 지원되지 않음 |
| Mbox 미리 가져오기 | 사용자 지정 범위 및 SPA VEC에 대해 지원됨 | 미리 가져오기는 Web SDK의 기본 모드입니다. |
...
| 응답 토큰 | 지원됨 | 지원됨. at.js와 Platform Web SDK 간의 코드 예제 및 차이점에 대해서는 [전용 응답 토큰 설명서](https://experienceleague.adobe.com/docs/target/using/administer/response-tokens.html)를 참조하십시오 |
...
| | Target at.js 2.x | Platform Web SDK |
|---|---|---|
| 플리커 완화 | 비동기 구현을 위한 코드 조각 사전 숨김에서 `at-body-style`의 스타일 ID를 사용합니다. at.js는 응답이 수신된 후 스타일을 제거하기 위해 이 요소 ID를 찾습니다. | 기본 코드 조각 사전 숨김은 `alloy-prehiding`의 스타일 ID를 사용합니다. 웹 SDK은 코드 조각을 미리 숨기는 at.js와 호환되지 않으므로 마이그레이션 프로세스의 일부로 변경해야 합니다. |
...
| 수동으로 컨텐츠 렌더링 | `applyOffer()` 및 `applyOffers()` 함수는 HTML 설정만 지원합니다. | `applyPropositions` 명령은 유연성을 높이기 위해 HTML의 설정, 바꾸기 또는 추가를 지원합니다 |
| 사용자 지정 이벤트 추적 | `trackEvent()` 및 `sendNotifications()` 함수로 지원됩니다. 이러한 함수는 Target에만 해당되며 Adobe Analytics 지표에 영향을 주지 않습니다. | Platform Web SDK `sendEvent` 호출의 모든 데이터가 Target으로 전달됩니다. Adobe Analytics 지표가 영향을 받지 않도록 하려면 Target에 특별히 필요한 보조 데이터를 eventType이 `sendEvent` 또는 `decisioning.propositionDisplay`인 `decisioning.propositionInteract` 명령에 포함해야 합니다. |
...
많은 at.js 함수에는 아래 표에 요약된 Platform Web SDK을 사용하는 것과 동일한 접근 방식이 있습니다. [at.js 함수](https://developer.adobe.com/target/implement/client-side/atjs/atjs-functions/atjs-functions/)에 대한 자세한 내용은 Adobe Target 개발자 안내서를 참조하십시오.

| at.js 2.x 함수 | Platform Web SDK 동등 항목 |
| --- | --- |
| `getOffer()` 및 `getOffers()` | Target VEC 기반 경험을 요청하고 [자동으로 렌더링](https://experienceleague.adobe.com/docs/experience-platform/edge/personalization/rendering-personalization-content.html#automatically-rendering-content)하려면 `sendEvent` 명령을 사용하고 `renderDecisions` 옵션을 true로 설정하십시오.<br><br>양식 기반 경험을 요청하거나 [수동으로 렌더링](https://experienceleague.adobe.com/docs/experience-platform/edge/personalization/rendering-personalization-content.html#manually-rendering-content) 콘텐츠를 요청하려면 `decisionScopes` 명령을 사용하여 `sendEvent`(mbox)의 배열을 지정하십시오. |
| `applyOffer()` 및 `applyOffers()` | 콘텐츠를 적용하려면 [`applyPropositions`](https://experienceleague.adobe.com/docs/experience-platform/edge/personalization/rendering-personalization-content.html#applypropositions) 명령을 사용하십시오. HTML을 특정 선택기로 설정, 대체 또는 추가하도록 선택할 수 있습니다. |
| `triggerView()` | [ 명령의 ](https://experienceleague.adobe.com/docs/experience-platform/edge/personalization/adobe-target/web-sdk-atjs-comparison.html#how-to-trigger-a-view-change-in-a-single-page-application) 옵션 아래에 `web.webPageDetails.viewName` 속성이 설정된 경우 SPA VEC를 위해 Platform Web SDK에서 `xdm`보기 변경`sendEvent`을 자동으로 트리거합니다. |
...
| --- | --- |
| `bodyHiddenStyle` | [`prehidingStyle` 명령을 사용하여 ](https://experienceleague.adobe.com/docs/experience-platform/edge/fundamentals/configuring-the-sdk.html#prehidingStyle)`configure` 설정 |
...
| `crossDomain` | `thirdPartyCookiesEnabled` 명령을 사용하여 `true` 옵션을 `configure`(으)로 설정하여 도메인 간 사용 사례에 대해 자사 및 서드파티 쿠키를 사용하도록 설정합니다. |
...
| `deviceIdLifetime` | 지원되지 않습니다. `targetMigrationEnabled` 명령을 사용하여 `true`을(를) `configure`(으)로 설정하면 장치 수명이 2년으로 설정된 `mbox` 쿠키가 설정됩니다. 이 값은 구성할 수 없습니다. |
| `enabled` | 대상 기능은 데이터 스트림 구성으로 활성화되거나 비활성화됩니다. |
| `globalMboxAutoCreate` | VEC 기반 경험을 자동으로 가져오고 렌더링하려면 `renderDecisions` 명령을 사용하여 `true` 옵션을 `sendEvent`(으)로 설정하십시오.VEC 기반 경험을 수동으로 렌더링하려면 <br><br>`decisionScope`에 대해 `__view__`을(를) 요청합니다. |
| `imsOrgId` | `orgId` 명령으로 `configure` 설정 |
...
| `pageLoadEnabled` | `renderDecisions` 명령을 사용하여 `true` 옵션을 `sendEvent`(으)로 설정 |
| `secureOnly` | 지원되지 않습니다. Platform Web SDK은 `secure` 및 `sameSite="none"` 특성이 있는 모든 쿠키를 설정합니다. |
| `selectorsPollingTimeout` | 지원되지 않습니다. Platform Web SDK은 5초 값을 사용합니다. 필요한 경우 사용자 지정 코드를 사용하여 콘텐츠를 수동으로 렌더링할 수 있습니다. |
| `serverDomain` | `edgeDomain` 명령과 함께 `configure` 설정 사용 |
| `telemetryEnabled` | 적용할 수 없음 |
| `timeout` | 지원되지 않습니다. 깜박임 완화 코드에 적절한 시간 제한이 포함되어 있는지 확인하는 것이 좋습니다. |
| `viewsEnabled` | 지원되지 않습니다. `sendEvent()`이(가) `renderDecisions`(으)로 설정되어 있거나 `true` decisionScope가 요청에 포함된 경우 Target 보기의 콘텐츠를 항상 첫 번째 `__view__` 호출 시 가져옵니다. |
| `visitorApiTimeout` | 적용할 수 없음 |


## 시스템 다이어그램 비교

다음 다이어그램은 at.js를 사용하는 Target 구현과 Platform Web SDK을 사용하는 구현 간의 데이터 흐름 차이를 이해하는 데 도움이 됩니다.
...
| 6 | 타깃팅된 컨텐츠를 다시 페이지로 전송하며, 원할 경우 추가적인 개인화를 위한 프로필 값을 포함할 수 있습니다.<br><br>현재 페이지의 타기팅된 콘텐츠는 기본 콘텐츠의 플리커 없이 가능한 한 빨리 나타납니다.<br><br>단일 페이지 응용 프로그램의 미래 보기를 위한 타깃팅된 콘텐츠가 브라우저에 캐시되므로 보기가 트리거될 때 추가적인 서버 호출 없이 즉시 적용될 수 있습니다. |
...
![Platform Web SDK을 사용한 Adobe Target Edge Decisioning 다이어그램](assets/target-platform-web-sdk.png)

| 호출 | 세부 사항 |
| --- | --- |
| 1 | 장치가 Platform Web SDK을 로드합니다. Platform 웹 SDK은 XDM 데이터, 데이터스트림 환경 ID, 전달된 매개 변수 및 고객 ID(선택 사항)가 있는 Edge Network에 요청을 보냅니다. 페이지(또는 컨테이너)가 미리 숨겨져 있습니다. |
...
[단일 페이지 애플리케이션용 Platform Web SDK을 사용하여 Target을 구현](https://experienceleague.adobe.com/docs/experience-platform/edge/personalization/adobe-target/spa-implementation.html)하는 방법에 대한 자세한 내용은 개발자 안내서를 참조하십시오.
