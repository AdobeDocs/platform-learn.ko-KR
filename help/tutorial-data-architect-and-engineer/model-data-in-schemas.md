...
>[!NOTE]
>
> `meta:altId`CONTAINER_ID **[!DNL Schema Registry API > Schemas > Retrieve a list of schemas within the specified container.]**&#x200B;이(가) **[!UICONTROL (으)로 설정되고 허용 헤더]**&#x200B;이(가) 있는 API 요청 `tenant`을(를) 수행하여 `application/vnd.adobe.xdm+json` 또는 스키마 ID를 가져올 수도 있습니다.
...
1. `$id`의 `Luma Identity profile field group` 값을 가져옵니다(현재 값은 이 스크린샷의 값과 다름).
...
1. **본문** 탭을 열고 다음 코드를 붙여 넣어 `$ref` 값을 `$id`의 `Luma Identity profile field group`(으)로 바꿉니다.
...
**[!UICONTROL 소비자 경험 이벤트]** 필드 그룹을 선택하십시오. 이 필드 그룹에는 [!UICONTROL Commerce 세부 정보]에 있던 commerce 및 productListItems 개체가 포함되어 있습니다. [!UICONTROL 고객 경험 이벤트]는 별도로 사용할 수 있는 여러 다른 표준 필드 그룹의 조합입니다. [!UICONTROL AEP Web SDK ExperienceEvent] 필드 그룹에는 [!UICONTROL 고객 경험 이벤트]에 있는 동일한 필드 그룹을 포함하여 다른 필드 그룹도 포함되어 있습니다. 다행히도, 그들은 완벽하게 혼합됩니다.
