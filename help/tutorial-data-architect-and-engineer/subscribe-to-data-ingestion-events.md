...
>[!IMPORTANT]
>
> 데이터 수집 이벤트에 의해 트리거되는 이러한 알림은 _뿐만 아니라_&#x200B;모든 샌드박스`Luma Tutorial`에 적용됩니다. 계정의 다른 데이터 수집 이벤트에서 시작된 알림도 표시될 수 있습니다.
...
1. 위쪽 탐색에서 **편집** 단추 선택
1. 응답 본문으로 `$request.query.challenge$`을(를) 입력하십시오. 이 단원의 뒷부분에서 설정한 Adobe I/O Events 알림은 웹후크에 문제를 보내며, 이를 응답 본문에 포함해야 합니다.
1. **저장** 단추 선택
