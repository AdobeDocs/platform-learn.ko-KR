...
1. 페이지의 **[!UICONTROL 사용해 보기]** 섹션에서 **[!UICONTROL Postman용 다운로드]**&#x200B;를 선택한 다음 **[!UICONTROL OAuth 서버 간 다운로드]**&#x200B;를 선택하여 [!DNL Postman] 환경 json 파일을 다운로드합니다. `oauth_server_to_server.postman_environment.json` 폴더에 `Luma Tutorial Assets`을(를) 저장합니다.
...
1. 목록이 길면 튜토리얼 참가자가 제공한 `Credential in Luma Tutorial API Project`기술 계정 전자 메일[!UICONTROL 로 필터링하여 ] 자격 증명을 찾습니다
1. 자격 증명 선택
1. **[!UICONTROL 저장]** 선택
...
Adobe에서는 Experience Platform API를 탐색하는 데 도움이 되는 다양한 [!DNL Postman] 컬렉션 집합을 제공합니다. 이러한 컬렉션은 [Adobe Experience Platform Postman 샘플 GitHub 저장소](https://github.com/adobe/experience-platform-postman-samples)에 있습니다. 이 자습서를 통해 여러 번 사용하고 나중에 회사에 대해 Experience Platform을 구현할 때 이 저장소를 책갈피로 지정해야 합니다.

첫 번째 컬렉션은 Adobe IMS(Identity Management Service) API와 함께 작동합니다. Postman 내에서 액세스 토큰을 검색하는 편리한 방법입니다.

액세스 토큰을 생성하려면:

1. [ 폴더에 ](https://github.com/adobe/experience-platform-postman-samples/blob/master/apis/ims/Identity%20Management%20Service.postman_collection.json)Identity Management 서비스 API 컬렉션`Luma Tutorial Assets`을(를) 다운로드합니다.
...
1. 액세스 토큰은 **환경의** ACCESS_TOKEN[!DNL Postman] 환경 변수로 자동으로 저장되어야 합니다.
...
GitHub에서 [Experience Platform [!DNL Postman] 컬렉션](https://github.com/adobe/experience-platform-postman-samples/tree/master/apis/experience-platform)을 엽니다. 이 페이지에는 다양한 Platform API에 대한 많은 컬렉션이 있습니다. 북마킹을 강력히 추천합니다

이제 첫 번째 API 호출을 만들어 보겠습니다.

1. [ 폴더에 ](https://raw.githubusercontent.com/adobe/experience-platform-postman-samples/master/apis/experience-platform/Schema%20Registry%20API.postman_collection.json)스키마 레지스트리 API 컬렉션`Luma Tutorial Assets`을(를) 다운로드합니다.
