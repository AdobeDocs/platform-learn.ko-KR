---
title: 쿼리 서비스 - 쿼리, 쿼리, 쿼리, 쿼리.. 및 이탈 분석
description: 쿼리 서비스 - 쿼리, 쿼리, 쿼리, 쿼리.. 및 이탈 분석
kt: 5342
...
# 4.3 쿼리, 쿼리, 쿼리... 및 이탈 분석

## 목표

* 데이터 분석에 대한 쿼리 쓰기
* Adobe Experience Platform에서 사용할 수 있는 온라인, 콜 센터 및 충성도 데이터를 결합하여 SQL 쿼리를 작성합니다
* Adobe 정의 기능에 대해 알아보기

## 컨텍스트

이 연습에서는 제품 보기, 제품 유입 경로, 이탈 등을 분석하는 쿼리를 작성합니다.

이 장에 나열된 모든 쿼리는 **PSQL 명령줄 인터페이스**. 다음 방법으로 표시된 문 블록을 복사해야 합니다(CTRL-c) **SQL** 에 (CTRL-v)붙여넣습니다. **PSQL 명령줄 인터페이스**. 다음 **쿼리 결과** 블록은 붙여넣은 SQL 문과 관련 쿼리 결과를 보여 줍니다.

## 4.3.1 데이터 분석을 위한 기본 쿼리 작성

### 타임스탬프

Adobe Experience Platform에 캡처된 데이터는 타임스탬프가 지정됩니다. 다음 **timestamp** 속성을 사용하면 시간에 따른 데이터를 분석할 수 있습니다.
...
### 이탈할 위험이 있는 방문자 식별(방문 페이지 => 서비스 취소)
...
다음 쿼리 세트에서 Adobe는 위의 쿼리를 확장하여 &quot;서비스 취소&quot; 페이지를 방문한 고객 및 해당 행동에 대한 전체 보기를 가져옵니다. Adobe 정의 함수를 사용하여 정보를 세션화하고, 이벤트의 시퀀스 및 타이밍을 식별하는 방법을 알아봅니다. 또한 데이터 세트를 함께 결합하여 Microsoft Power BI에서 데이터를 더 풍부하게 만들고 분석할 수 있도록 준비합니다.

## 4.3.2 고급 쿼리

비즈니스 로직의 대부분은 고객의 터치 포인트를 수집하고 시간별로 주문해야 합니다. 이 지원은 Spark SQL에서 창 함수 형태로 제공합니다. 창 함수는 표준 SQL의 일부이며 다른 많은 SQL 엔진에서 지원됩니다.

### Adobe 정의 함수

Adobe이 일련의 **Adobe 정의 함수** 경험 데이터를 더 잘 이해할 수 있도록 표준 SQL 구문에 추가합니다. 다음 두 가지 쿼리에서 이러한 ADF 함수에 대해 학습합니다. 자세한 정보와 전체 목록을 찾을 수 있습니다 [설명서에서](https://experienceleague.adobe.com/docs/experience-platform/query/sql/adobe-defined-functions.html).

### 세션에서 세 번째 페이지로 &quot;서비스 취소&quot; 페이지에 도달하기 전에 사이트에서 수행하는 작업은 무엇입니까?

이 쿼리를 사용하면 처음 두 Adobe 정의 함수를 살펴봅니다 **SESS_TIMEOUT** 및 **다음**

> 다음 **SESS_TIMEOUT()** Adobe Analytics으로 찾은 방문 그룹을 재생성합니다. 비슷한 시간 기반 그룹화를 수행하지만 사용자 지정 가능한 매개 변수는 수행합니다.
>
> **NEXT()** 및 **PREVIOUS()** 고객이 사이트를 탐색하는 방법을 이해하는 데 도움이 됩니다.
...
### 방문자가 &quot;서비스 취소&quot; 페이지를 방문한 후 콜 센터에 전화하기 전에 시간이 얼마나 남았습니까?

이러한 종류의 쿼리에 응답하려면 **TIME_BETWEEN_NEXT_MATCH()** Adobe 정의 함수 를 참조하십시오.

> 이전 또는 다음 일치 함수 간 시간은 특정 사고 이후 경과된 시간을 측정하는 새 차원을 제공합니다.
...
데이터 세트를 함께 결합하고 있다는 점을 설명하고 이 경우 Adobe에 참여합니다 `demo_system_event_dataset_for_website_global_v1_1` with `demo_system_event_dataset_for_call_center_global_v1_1`. 우리는 콜센터 상호 작용의 결과를 알기 위해 이렇게 합니다.
...
### 이러한 고객의 충성도 프로필이란 무엇입니까?

이 쿼리에서 Adobe Experience Platform에서 온보딩한 충성도 데이터를 결합합니다. 이를 통해 충성도 데이터를 사용하여 이탈 분석을 보강할 수 있습니다.
...
추교 고객에 대한 일부 지리적 통찰력을 얻기 위해 Adobe Experience Platform에서 캡처한 경도, 태도, 도시, 국가 코드와 같은 지리적 정보를 포함할 수 있습니다.
...
위의 쿼리는 서비스 취소가 발생한 경우 콜 센터에 연락하게 된 방문자만 열람했습니다. 우리는 이것을 좀 더 광범위하게 받아들여 (wifi, 프로모션, 송장, 불만 및 계약)을 포함한 모든 콜 센터 상호 작용을 고려하려고 합니다.

쿼리를 편집해야 하므로 먼저 메모장 또는 대괄호를 엽니다.

Windows의 경우 Windows 도구 모음에서 &quot;search&quot;-icon(1)을 클릭하여 **메모장** &quot;search&quot;-field(2)에서 (3) &quot;메모장&quot; 결과를 클릭합니다.
...
제거 안 함 `/\*` 및 `\*/`. 메모장의 수정한 문은 다음과 같습니다.

![edit-query-notepad.png](./images/edit-query-notepad.png)

수정한 다음 **메모장** 로 **PSQL 명령줄 창** 을 눌러 enter 키를 누릅니다. PSQL 명령줄 창에 다음 결과가 표시됩니다.
...
그 다음 에서는 쿼리를 유지합니다( **선택한 대로 테이블 만들기** 또는 **CTAS**)을 Microsoft Power BI에서 사용할 새 데이터 세트로 사용합니다.

다음 단계: [4.4 쿼리에서 데이터 세트 생성](./ex4.md)
